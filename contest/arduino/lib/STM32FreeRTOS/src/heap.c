 ...
 * @brief   Provide Memory allocation implementations included in the FreeRTOS
 * source heap_useNewlib_ST - thread-safe memory manager using C runtime
 * (Newlib) heap_1 - the very simplest, does not permit memory to be freed
 * heap_2 - permits memory to be freed, but not does coalescence adjacent free
 * blocks. heap_3 - simply wraps the standard malloc() and free() for thread
 * safety heap_4 - coalescences adjacent free blocks to avoid fragmentation.
 * Includes absolute address placement option heap_5 - as per heap_4, with the
 * ability to span the heap across multiple non-adjacent memory areas
 */
#include "FreeRTOS.h"
