/* --------------------------------------------------------------------------
 * Portions Copyright © 2017 STMicroelectronics International N.V. All rights
 *reserved. Portions Copyright (c) 2013-2017 ARM Limited. All rights reserved.
 * --------------------------------------------------------------------------
 *
...
typedef enum {
  ...
  osKernelReserved =
      0x7FFFFFFFU ///< Prevents enum down-size compiler optimization.
} osKernelState_t;
...
typedef struct {
  ...
  osPriority_t
      priority; ///< initial thread priority (default: osPriorityNormal)
...
/// \param[out]    version       pointer to buffer for retrieving version
/// information. \param[out]    id_buf        pointer to buffer for retrieving
/// kernel identification string. \param[in]     id_size       size of buffer
/// for kernel identification string. \return status code that indicates the
...
/// \return previous lock state (1 - locked, 0 - not locked, error code if
/// negative).
...
/// \param[in]     lock          lock state obtained by \ref osKernelLock or
/// \ref osKernelUnlock. \return new lock state (1 - locked, 0 - not locked,
...
/// \param[in]     sleep_ticks   time in ticks for how long the system was in
...
/// \param[in]     argument      pointer that is passed to the thread function
/// as start argument. \param[in]     attr          thread attributes; NULL:
/// default values. \return thread ID for reference by other functions or NULL
/// in case of error.
...
/// osThreadGetId. \return name as NULL terminated string.
...
/// \param[in]     thread_id     thread ID obtained by \ref osThreadNew or \ref
/// osThreadGetId. \return current thread state of the specified thread.
osThreadState_t osThreadGetState(osThreadId_t thread_id);

/// Get stack size of a thread.
/// \param[in]     thread_id     thread ID obtained by \ref osThreadNew or \ref
/// osThreadGetId. \return stack size in bytes.
uint32_t osThreadGetStackSize(osThreadId_t thread_id);

/// Get available stack space of a thread based on stack watermark recording
/// during execution. \param[in]     thread_id     thread ID obtained by \ref
/// osThreadNew or \ref osThreadGetId. \return remaining stack space in bytes.
uint32_t osThreadGetStackSpace(osThreadId_t thread_id);

/// Change priority of a thread.
/// \param[in]     thread_id     thread ID obtained by \ref osThreadNew or \ref
/// osThreadGetId. \param[in]     priority      new priority value for the
...
/// \param[in]     thread_id     thread ID obtained by \ref osThreadNew or \ref
/// osThreadGetId. \return current priority value of the specified thread.
...
/// \param[in]     thread_id     thread ID obtained by \ref osThreadNew or \ref
/// osThreadGetId. \param[in]     flags         specifies the flags of the
/// thread that shall be set. \return thread flags after setting or error code
/// if highest bit set.
uint32_t osThreadFlagsSet(osThreadId_t thread_id, uint32_t flags);

/// Clear the specified Thread Flags of current running thread.
/// \param[in]     flags         specifies the flags of the thread that shall be
/// cleared. \return thread flags before clearing or error code if highest bit
/// set.
...
/// signaled. \param[in]     flags         specifies the flags to wait for.
/// \param[in]     options       specifies flags options (osFlagsXxxx).
/// \param[in]     timeout       \ref CMSIS_RTOS_TimeOutValue or 0 in case of no
/// time-out. \return thread flags before clearing or error code if highest bit
/// set.
...
/// \param[in]     type          \ref osTimerOnce for one-shot or \ref
/// osTimerPeriodic for periodic behavior. \param[in]     argument      argument
/// to the timer callback function. \param[in]     attr          timer
