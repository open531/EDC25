...
// 角度初始化
const uint8_t msgInitAngle[3] = {0xFF, 0xAA, 0x52};

// 加速度计校准
const uint8_t msgCalibrateAcce[3] = {0xFF, 0xAA, 0x67};

// 波特率115200，回传速率100Hz
const uint8_t msgSetBaud115200[3] = {0xFF, 0xAA, 0x63};

// 波特率9600，回传速率20Hz
const uint8_t msgSetBaud9600[3] = {0xFF, 0xAA, 0x64};

// 水平安装
const uint8_t msgSetHorizontal[3] = {0xFF, 0xAA, 0x65};

// 垂直安装
const uint8_t msgSetVertical[3] = {0xFF, 0xAA, 0x66};

// 休眠
const uint8_t msgSleepAndAwake[3] = {0xFF, 0xAA, 0x60};

class JY62 {
public:
  JY62(HardwareSerial *serial, int baud); // 构造函数
  ...
  bool isFreshValid(void); // 判断是否有新的有效数据
  void printData(uint8_t type, HardwareSerial &serial);
