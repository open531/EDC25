...
/** @defgroup PWR_Regulator_state_in_SLEEP_STOP_mode PWR Regulator state in
 * SLEEP/STOP mode
 * @{
 */
 ...
 * event was received from the WKUP pin or from the RTC alarm An additional
 * wakeup event is detected if the WKUP pin is enabled (by setting the EWUP bit)
 ...
 *            @arg PWR_FLAG_PVDO: PVD Output. This flag is valid only if PVD is
 * enabled by the HAL_PWR_EnablePVD() function. The PVD is stopped by Standby
 * mode For this reason, this bit is equal to 0 after Standby or reset until the
 * PVDE bit is set.
...
#define __HAL_PWR_PVD_EXTI_DISABLE_EVENT()                                     \
  CLEAR_BIT(EXTI->EMR, PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_ENABLE_FALLING_EDGE()                               \
  SET_BIT(EXTI->FTSR, PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_DISABLE_FALLING_EDGE()                              \
  CLEAR_BIT(EXTI->FTSR, PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_ENABLE_RISING_EDGE()                                \
  SET_BIT(EXTI->RTSR, PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_GENERATE_SWIT()                                     \
  SET_BIT(EXTI->SWIER, PWR_EXTI_LINE_PVD)
/**
 * @}
 */
