/**
  ...
  *
  ...
  been inserted in each __HAL_RCC_PPP_CLK_ENABLE() macro.
...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization
  ...
  *
  ...
  ===============================================================================
    [..]
      This section provides functions allowing to configure the
  internal/external oscillators (HSE, HSI, LSE, LSI, PLL, CSS and MCO) and the
  System buses clocks (SYSCLK, AHB, APB1 and APB2).

    [..] Internal/external clock and PLL configuration
      (#) HSI (high-speed internal), 8 MHz factory-trimmed RC used directly or
  through the PLL as System clock source.
      (#) LSI (low-speed internal), ~40 KHz low consumption RC used as IWDG
  and/or RTC clock source.

      (#) HSE (high-speed external), 4 to 24 MHz (STM32F100xx) or 4 to 16 MHz
  (STM32F101x/STM32F102x/STM32F103x) or 3 to 25 MHz (STM32F105x/STM32F107x)
  crystal oscillator used directly or through the PLL as System clock source.
  Can be used also as RTC clock source.

      (#) LSE (low-speed external), 32 KHz oscillator used as RTC clock source.

      (#) PLL (clocked by HSI or HSE), featuring different output clocks:
        (++) The first output is used to generate the high speed system clock
  (up to 72 MHz for STM32F10xxx or up to 24 MHz for STM32F100xx)
  ...
  through PLL as System clock source), the System clocks automatically switched
  to HSI and an interrupt is generated if enabled. The interrupt is linked to
  the Cortex-M3 NMI (Non-Maskable Interrupt) exception vector.

      (#) MCO1 (microcontroller clock output), used to output SYSCLK, HSI,
          HSE or PLL clock (divided by 2) on PA8 pin + PLL2CLK, PLL3CLK/2,
  PLL3CLK and XTI for STM32F105x/STM32F107x

    [..] System, AHB and APB buses clocks configuration
      (#) Several clock sources can be used to drive the System clock (SYSCLK):
  HSI, HSE and PLL. The AHB clock (HCLK) is derived from System clock through
  configurable prescaler and used to clock the CPU, memory and peripherals
  mapped on AHB bus (DMA, GPIO...). APB1 (PCLK1) and APB2 (PCLK2) clocks are
  derived from AHB clock through configurable prescalers and used to clock the
  peripherals mapped on these buses. You can use "HAL_RCC_GetSysClockFreq()"
  function to retrieve the frequencies of these clocks.

      -@- All the peripheral clocks are derived from the System clock (SYSCLK)
  except:
          (+@) RTC: RTC clock can be derived either from the LSI, LSE or HSE
  clock divided by 128.
          (+@) USB OTG FS and RTC: USB OTG FS require a frequency equal to 48
  MHz to work correctly. This clock is derived of the main PLL through PLL
  Multiplier.
          (+@) I2S interface on STM32F105x/STM32F107x can be derived from
  PLL3CLK
          (+@) IWDG clock which is always the LSI clock.

      (#) For STM32F10xxx, the maximum frequency of the SYSCLK and HCLK/PCLK2 is
  72 MHz, PCLK1 36 MHz. For STM32F100xx, the maximum frequency of the SYSCLK and
  HCLK/PCLK1/PCLK2 is 24 MHz. Depending on the SYSCLK frequency, the flash
  latency should be adapted accordingly.
  @endverbatim
  * @{
  */
 ...
 * @note   The PLL is not disabled when USB OTG FS clock is enabled (specific to
 * devices with USB FS)
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
  ...
  if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI) ==
    ...
    if (RCC_OscInitStruct->LSIState != RCC_LSI_OFF) {
      ...
      /*  To have a fully stabilized clock in the specified range, a software
         delay of 1ms should be added.*/
      RCC_Delay(1);
    } else {
  ...
  if ((RCC_OscInitStruct->PLL2.PLL2State) != RCC_PLL2_NONE) {
    /* This bit can not be cleared if the PLL2 clock is used indirectly as
      system clock (i.e. it is used as PLL clock entry that is used as system
      clock). */
    if ((__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSE) &&
        (__HAL_RCC_GET_SYSCLK_SOURCE() == RCC_SYSCLKSOURCE_STATUS_PLLCLK) &&
        ((READ_BIT(RCC->CFGR2, RCC_CFGR2_PREDIV1SRC)) ==
      ...
      if ((RCC_OscInitStruct->PLL2.PLL2State) == RCC_PLL2_ON) {
        ...
        assert_param(
            IS_RCC_HSE_PREDIV2(RCC_OscInitStruct->PLL2.HSEPrediv2Value));

        /* Prediv2 can be written only when the PLLI2S is disabled. */
        /* Return an error only if new value is different from the programmed
         * value */
  ...
  if ((RCC_OscInitStruct->PLL.PLLState) != RCC_PLL_NONE) {
    ...
    if (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_SYSCLKSOURCE_STATUS_PLLCLK) {
      if ((RCC_OscInitStruct->PLL.PLLState) == RCC_PLL_ON) {
        ...
        /* It can be written only when the PLL is disabled. Not used in PLL
         * source is different than HSE */
 ...
 *          The value of this parameter depend on device used within the same
 * series
 * @note   The SystemCoreClock CMSIS variable is used to store System Clock
 * Frequency and updated by @ref HAL_RCC_GetHCLKFreq() function called within
 * this function
 *
 ...
 *         clock source is ready (clock stable after start-up delay or PLL
 * locked). If a clock source which is not yet ready is selected, the switch
 * will occur when the clock source will be ready. You can use @ref
 * HAL_RCC_GetClockConfig() function to know which clock is currently used as
 * system clock source.
 * @retval HAL status
 */
...
/**
  ...
  *            @arg @ref RCC_MCO1SOURCE_PLL2CLK      PLL2 clock selected as MCO
  source
  *            @arg @ref RCC_MCO1SOURCE_PLL3CLK_DIV2 PLL3 clock divided by 2
  selected as MCO source
  *            @arg @ref RCC_MCO1SOURCE_EXT_HSE      XT1 external 3-25 MHz
  oscillator clock selected as MCO source
  *            @arg @ref RCC_MCO1SOURCE_PLL3CLK      PLL3 clock selected as MCO
  source
  @endif
  @if STM32F107xC
  *            @arg @ref RCC_MCO1SOURCE_PLLCLK       PLL clock divided by 2
 ...
 * HSI_VALUE(*)
 * @note     If SYSCLK source is HSE, function returns a value based on
 * HSE_VALUE divided by PREDIV factor(**)
 * @note     If SYSCLK source is PLL, function returns a value based on
 * HSE_VALUE divided by PREDIV factor(**) or HSI_VALUE(*) multiplied by the PLL
 * factor.
 * @note     (*) HSI_VALUE is a constant defined in stm32f1xx_hal_conf.h file
 * (default value 8 MHz) but the real value may vary depending on the variations
 *               in voltage and temperature.
 * @note     (**) HSE_VALUE is a constant defined in stm32f1xx_hal_conf.h file
 ...
 *         baud-rate for the communication peripherals or configure other
 * parameters.
 *
...
#if defined(RCC_CFGR2_PREDIV1SRC)
  static const uint8_t aPLLMULFactorTable[14U] = {0, 0, 4, 5, 6, 7, 8,
                                                  9, 0, 0, 0, 0, 0, 13};
  ...
  case RCC_SYSCLKSOURCE_STATUS_PLLCLK: /* PLL used as system clock */
  {
    ...
    if ((tmpreg & RCC_CFGR_PLLSRC) != RCC_PLLSOURCE_HSI_DIV2) {
#if defined(RCC_CFGR2_PREDIV1)
      prediv = aPredivFactorTable[(uint32_t)(RCC->CFGR2 & RCC_CFGR2_PREDIV1) >>
                                  RCC_CFGR2_PREDIV1_Pos];
#else
      prediv = aPredivFactorTable[(uint32_t)(RCC->CFGR & RCC_CFGR_PLLXTPRE) >>
      ...
      if (HAL_IS_BIT_SET(RCC->CFGR2, RCC_CFGR2_PREDIV1SRC)) {
        ...
        /* PLLCLK = PLL2CLK / PREDIV1 * PLLMUL with PLL2CLK = HSE/PREDIV2 *
         * PLL2MUL */
        prediv2 =
            ((RCC->CFGR2 & RCC_CFGR2_PREDIV2) >> RCC_CFGR2_PREDIV2_Pos) + 1;
        pll2mul =
            ((RCC->CFGR2 & RCC_CFGR2_PLL2MUL) >> RCC_CFGR2_PLL2MUL_Pos) + 2;
        pllclk = (uint32_t)(((uint64_t)HSE_VALUE * (uint64_t)pll2mul *
                             (uint64_t)pllmul) /
                            ((uint64_t)prediv2 * (uint64_t)prediv));
      } else {
        /* HSE used as PLL clock source : PLLCLK = HSE/PREDIV1 * PLLMUL */
        pllclk = (uint32_t)((HSE_VALUE * pllmul) / prediv);
      }

      /* If PLLMUL was set to 13 means that it was to cover the case PLLMUL 6.5
       * (avoid using float) */
      /* In this case need to divide pllclk by 2 */
      if (pllmul == aPLLMULFactorTable[(uint32_t)(RCC_CFGR_PLLMULL6_5) >>
                                       RCC_CFGR_PLLMULL_Pos]) {
        pllclk = pllclk / 2;
      }
#else
 ...
 *
 * @note   The SystemCoreClock CMSIS variable is used to store System Clock
 * Frequency and updated within this function
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
  ...
  RCC_OscInitStruct->HSICalibrationValue =
      (uint32_t)((RCC->CR & RCC_CR_HSITRIM) >> RCC_CR_HSITRIM_Pos);
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
  ...
  RCC_ClkInitStruct->APB2CLKDivider =
      (uint32_t)((RCC->CFGR & RCC_CFGR_PPRE2) >> 3);
