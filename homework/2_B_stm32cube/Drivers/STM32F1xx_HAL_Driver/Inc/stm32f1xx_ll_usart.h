...
typedef struct {
  ...
  uint32_t
      ClockPhase; /*!< Specifies the clock transition on which the bit capture
                     is made. This parameter can be a value of @ref
                     USART_LL_EC_PHASE.
 ...
 * case
 */
#define __LL_USART_DIV_SAMPLING8_100(__PERIPHCLK__, __BAUDRATE__)              \
  (((__PERIPHCLK__)*25) / (2 * (__BAUDRATE__)))
#define __LL_USART_DIVMANT_SAMPLING8(__PERIPHCLK__, __BAUDRATE__)              \
  (__LL_USART_DIV_SAMPLING8_100((__PERIPHCLK__), (__BAUDRATE__)) / 100)
#define __LL_USART_DIVFRAQ_SAMPLING8(__PERIPHCLK__, __BAUDRATE__)              \
  (((__LL_USART_DIV_SAMPLING8_100((__PERIPHCLK__), (__BAUDRATE__)) -           \
     (__LL_USART_DIVMANT_SAMPLING8((__PERIPHCLK__), (__BAUDRATE__)) * 100)) *  \
        8 +                                                                    \
    50) /                                                                      \
   100)
/* UART BRR = mantissa + overflow + fraction
            = (UART DIVMANT << 4) + ((UART DIVFRAQ & 0xF8) << 1) + (UART DIVFRAQ
   & 0x07) */
#define __LL_USART_DIV_SAMPLING8(__PERIPHCLK__, __BAUDRATE__)                  \
  (((__LL_USART_DIVMANT_SAMPLING8((__PERIPHCLK__), (__BAUDRATE__)) << 4) +     \
    ((__LL_USART_DIVFRAQ_SAMPLING8((__PERIPHCLK__), (__BAUDRATE__)) & 0xF8)    \
     << 1)) +                                                                  \
 ...
 * case
 */
#define __LL_USART_DIV_SAMPLING16_100(__PERIPHCLK__, __BAUDRATE__)             \
  (((__PERIPHCLK__)*25) / (4 * (__BAUDRATE__)))
#define __LL_USART_DIVMANT_SAMPLING16(__PERIPHCLK__, __BAUDRATE__)             \
  (__LL_USART_DIV_SAMPLING16_100((__PERIPHCLK__), (__BAUDRATE__)) / 100)
#define __LL_USART_DIVFRAQ_SAMPLING16(__PERIPHCLK__, __BAUDRATE__)             \
  ((((__LL_USART_DIV_SAMPLING16_100((__PERIPHCLK__), (__BAUDRATE__)) -         \
      (__LL_USART_DIVMANT_SAMPLING16((__PERIPHCLK__), (__BAUDRATE__)) *        \
       100)) *                                                                 \
     16) +                                                                     \
    50) /                                                                      \
   100)
/* USART BRR = mantissa + overflow + fraction
            = (USART DIVMANT << 4) + (USART DIVFRAQ & 0xF0) + (USART DIVFRAQ &
   0x0F) */
 ...
 * USART is kept, but all the status flags, in the USARTx_SR are set to their
 * default values.
 ...
 * pin or not
 * @note   Macro @ref IS_USART_INSTANCE(USARTx) can be used to check whether or
 * not Synchronous mode is supported by the USARTx instance.
...
LL_USART_SetLastClkPulseOutput(USART_TypeDef *USARTx,
 ...
 * @note   only 4bits (b3-b0) of returned value are relevant (b31-b4 are not
 * relevant)
 ...
 * @note   Macro @ref IS_UART_HWFLOW_INSTANCE(USARTx) can be used to check
 * whether or not Hardware Flow control feature is supported by the USARTx
 * instance.
...
__STATIC_INLINE uint32_t LL_USART_GetBaudRate(const USART_TypeDef *USARTx,
  ...
  if (OverSampling == LL_USART_OVERSAMPLING_8) {
    if ((usartdiv & 0xFFF7U) != 0U) {
      usartdiv =
          (uint16_t)((usartdiv & 0xFFF0U) | ((usartdiv & 0x0007U) << 1U));
 ...
 * @note   Macro @ref IS_IRDA_INSTANCE(USARTx) can be used to check whether or
 * not IrDA feature is supported by the USARTx instance.
 ...
 * @note   Macro @ref IS_SMARTCARD_INSTANCE(USARTx) can be used to check whether
 * or not Smartcard feature is supported by the USARTx instance.
...
LL_USART_GetSmartcardGuardTime(const USART_TypeDef *USARTx) {
  return (uint32_t)(READ_BIT(USARTx->GTPR, USART_GTPR_GT) >>
 ...
 * @note   Macro @ref IS_UART_HALFDUPLEX_INSTANCE(USARTx) can be used to check
 ...
 * @note   Macro @ref IS_UART_LIN_INSTANCE(USARTx) can be used to check whether
 * or not LIN feature is supported by the USARTx instance.
 ...
 */
__STATIC_INLINE uint32_t
LL_USART_IsActiveFlag_RXNE(const USART_TypeDef *USARTx) {
 ...
 * NF=1 in the USARTx_SR register). 0: Interrupt is inhibited 1: An interrupt is
 ...
 */
__STATIC_INLINE uint32_t
LL_USART_IsEnabledIT_RXNE(const USART_TypeDef *USARTx) {
 ...
 * @note   Address of Data Register is valid for both Transmit and Receive
 * transfers.
 * @param  USARTx USART Instance
...
ErrorStatus
LL_USART_ClockInit(USART_TypeDef *USARTx,
