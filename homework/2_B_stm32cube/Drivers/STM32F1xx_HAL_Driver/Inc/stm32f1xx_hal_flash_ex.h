...
#define SR_FLAG_MASK                                                           \
  ((uint32_t)(FLASH_SR_BSY | FLASH_SR_PGERR | FLASH_SR_WRPRTERR | FLASH_SR_EOP))
...
#define IS_OPTIONBYTE(VALUE)                                                   \
  (((VALUE) <=                                                                 \
    (OPTIONBYTE_WRP | OPTIONBYTE_RDP | OPTIONBYTE_USER | OPTIONBYTE_DATA)))
...
#define IS_OB_BOOT1(BOOT1)                                                     \
  (((BOOT1) == OB_BOOT1_RESET) || ((BOOT1) == OB_BOOT1_SET))
...
#define IS_FLASH_NB_PAGES(ADDRESS, NBPAGES)                                    \
  (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x20U)                        \
       ? ((ADDRESS) + ((NBPAGES)*FLASH_PAGE_SIZE) - 1 <= 0x08007FFFU)          \
...
#define IS_FLASH_NB_PAGES(ADDRESS, NBPAGES)                                    \
  (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x80U)                        \
       ...
       : (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x40U)                 \
              ...
              : (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x20U)          \
                     ? ((ADDRESS) + ((NBPAGES)*FLASH_PAGE_SIZE) - 1 <=         \
                        0x08007FFFU)                                           \
                     : ((ADDRESS) + ((NBPAGES)*FLASH_PAGE_SIZE) - 1 <=         \
                        0x08003FFFU))))
...
#define IS_FLASH_NB_PAGES(ADDRESS, NBPAGES)                                    \
  (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x200U)                       \
       ...
       : (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x180U)                \
              ? ((ADDRESS) + ((NBPAGES)*FLASH_PAGE_SIZE) - 1 <= 0x0805FFFFU)   \
...
#if defined(FLASH_BANK2_END)
#define IS_FLASH_NB_PAGES(ADDRESS, NBPAGES)                                    \
  (((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0x400U)                       \
       ? ((ADDRESS) + ((NBPAGES)*FLASH_PAGE_SIZE) - 1 <= 0x080FFFFFU)          \
...
typedef struct {
  ...
  uint32_t PageAddress; /*!< PageAdress: Initial FLASH page address to erase
                           when mass erase is disabled This parameter must be a
                           number between Min_Data = 0x08000000 and Max_Data =
                           FLASH_BANKx_END (x = 1 or 2 depending on devices)*/

  uint32_t
      NbPages; /*!< NbPages: Number of pagess to be erased.
...
typedef struct {
  uint32_t
      OptionType; /*!< OptionType: Option byte to be configured.
  ...
  uint32_t
      Banks; /*!< Select banks for WRP activation/deactivation of all sectors.
                  This parameter must be a value of @ref FLASHEx_Banks */
...
#define FLASH_BANK_BOTH                                                        \
  ((uint32_t)FLASH_BANK_1 | FLASH_BANK_2) /*!< Bank1 and Bank2  */
...
#define OB_WRPSTATE_ENABLE                                                     \
  0x01U /*!<Enable the write protection of the desired pagess*/
...
#define OB_WRP_PAGES104TO107                                                   \
  0x04000000U /*!< Write protection of page 104 to 107 */
#define OB_WRP_PAGES108TO111                                                   \
  0x08000000U /*!< Write protection of page 108 to 111 */
#define OB_WRP_PAGES112TO115                                                   \
  0x10000000U /*!< Write protection of page 112 to 115 */
#define OB_WRP_PAGES116TO119                                                   \
  0x20000000U /*!< Write protection of page 115 to 119 */
#define OB_WRP_PAGES120TO123                                                   \
  0x40000000U /*!< Write protection of page 120 to 123 */
#define OB_WRP_PAGES124TO127                                                   \
  0x80000000U /*!< Write protection of page 124 to 127 */
#endif        /* STM32F100xB || STM32F101xB || STM32F102xB || STM32F103xB */
...
#define OB_WRP_PAGES62TO255                                                    \
  0x80000000U /*!< Write protection of page 62 TO 255 */
#define OB_WRP_PAGES62TO511                                                    \
  0x80000000U /*!< Write protection of page 62 TO 511 */
#endif        /* STM32F100xB || STM32F101xB || STM32F102xB || STM32F103xB */
              /* STM32F101xG || STM32F103xG */
              /* STM32F105xC || STM32F107xC */
...
#define OB_STOP_NO_RST                                                         \
  ((uint16_t)0x0002) /*!< No reset generated when entering in STOP */
#define OB_STOP_RST                                                            \
  ((uint16_t)0x0000) /*!< Reset generated when entering in STOP */
/**
 * @}
 */
...
#define OB_STDBY_NO_RST                                                        \
  ((uint16_t)0x0004) /*!< No reset generated when entering in STANDBY */
#define OB_STDBY_RST                                                           \
  ((uint16_t)0x0000) /*!< Reset generated when entering in STANDBY */
/**
 * @}
 */
...
#define FLASH_FLAG_BSY_BANK1                                                   \
  FLASH_SR_BSY /*!< FLASH Bank1 Busy flag                   */
#define FLASH_FLAG_PGERR_BANK1                                                 \
  FLASH_SR_PGERR /*!< FLASH Bank1 Programming error flag      */
#define FLASH_FLAG_WRPERR_BANK1                                                \
  FLASH_SR_WRPRTERR /*!< FLASH Bank1 Write protected error flag  */
#define FLASH_FLAG_EOP_BANK1                                                   \
  FLASH_SR_EOP /*!< FLASH Bank1 End of Operation flag       */

#define FLASH_FLAG_BSY_BANK2                                                   \
  (FLASH_SR2_BSY << 16U) /*!< FLASH Bank2 Busy flag                   */
#define FLASH_FLAG_PGERR_BANK2                                                 \
  (FLASH_SR2_PGERR << 16U) /*!< FLASH Bank2 Programming error flag      */
#define FLASH_FLAG_WRPERR_BANK2                                                \
  (FLASH_SR2_WRPRTERR << 16U) /*!< FLASH Bank2 Write protected error flag  */
#define FLASH_FLAG_EOP_BANK2                                                   \
  (FLASH_SR2_EOP << 16U) /*!< FLASH Bank2 End of Operation flag       */

#else

#define FLASH_FLAG_BSY                                                         \
  FLASH_SR_BSY /*!< FLASH Busy flag                          */
#define FLASH_FLAG_PGERR                                                       \
  FLASH_SR_PGERR /*!< FLASH Programming error flag             */
#define FLASH_FLAG_WRPERR                                                      \
  FLASH_SR_WRPRTERR /*!< FLASH Write protected error flag         */
#define FLASH_FLAG_EOP                                                         \
  FLASH_SR_EOP /*!< FLASH End of Operation flag              */

#endif
#define FLASH_FLAG_OPTVERR                                                     \
  ((OBR_REG_INDEX << 8U | FLASH_OBR_OPTERR)) /*!< Option Byte Error        */
/**
 * @}
 */
...
#define FLASH_IT_EOP_BANK1                                                     \
  FLASH_CR_EOPIE /*!< End of FLASH Operation Interrupt source Bank1 */
#define FLASH_IT_ERR_BANK1                                                     \
  FLASH_CR_ERRIE /*!< Error Interrupt source Bank1                  */

#define FLASH_IT_EOP_BANK2                                                     \
  (FLASH_CR2_EOPIE << 16U) /*!< End of FLASH Operation Interrupt source Bank2  \
                            */
#define FLASH_IT_ERR_BANK2                                                     \
  (FLASH_CR2_ERRIE << 16U) /*!< Error Interrupt source Bank2 */
...
#define FLASH_IT_ERR                                                           \
  FLASH_CR_ERRIE /*!< Error Interrupt source                  */
 ...
 *            @arg @ref FLASH_FLAG_EOP_BANK1    FLASH End of Operation flag on
 * bank1
 *            @arg @ref FLASH_FLAG_WRPERR_BANK1 FLASH Write protected error flag
 * on bank1
 *            @arg @ref FLASH_FLAG_PGERR_BANK1  FLASH Programming error flag on
 * bank1
 *            @arg @ref FLASH_FLAG_BSY_BANK1    FLASH Busy flag on bank1
 *            @arg @ref FLASH_FLAG_EOP_BANK2    FLASH End of Operation flag on
 * bank2
 *            @arg @ref FLASH_FLAG_WRPERR_BANK2 FLASH Write protected error flag
 * on bank2
 *            @arg @ref FLASH_FLAG_PGERR_BANK2  FLASH Programming error flag on
 * bank2
 *            @arg @ref FLASH_FLAG_BSY_BANK2    FLASH Busy flag on bank2
 *            @arg @ref FLASH_FLAG_OPTVERR  Loaded OB and its complement do not
 * match
 ...
 */
#define __HAL_FLASH_GET_FLAG(__FLAG__)                                         \
  (((__FLAG__) == FLASH_FLAG_OPTVERR) ? (FLASH->OBR & FLASH_OBR_OPTERR)        \
                                      : (FLASH->SR & (__FLAG__)))
/**
 * @brief  Clear the specified FLASH flag.
 * @param  __FLAG__ specifies the FLASH flags to clear.
