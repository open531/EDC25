...
typedef struct {
  ...
  __IOM uint32_t
      IP[8U]; /*!< Offset: 0x300 (R/W)  Interrupt Priority Register */
} NVIC_Type;
...
#define SCB_VTOR_TBLOFF_Msk                                                    \
  (0xFFFFFFUL << SCB_VTOR_TBLOFF_Pos) /*!< SCB VTOR: TBLOFF Mask */
#endif
...
#define MPU_RBAR_ADDR_Msk                                                      \
  (0xFFFFFFUL << MPU_RBAR_ADDR_Pos) /*!< MPU RBAR: ADDR Mask */
...
/**
  ...
  \brief    Cortex-M0+ Core Debug Registers (DCB registers, SHCSR, and DFSR) are
  only accessible over DAP and not via processor. Therefore they are not covered
  by the Cortex-M0+ header file.
  @{
 */
...
/*#define NVIC_GetActive              __NVIC_GetActive             not available
 * for Cortex-M0+ */
