...
static void buffer_scale_back_q15_to_q7(q15_t *buffer, q7_t *target,
...
static void
compare_and_replace_if_larger_q7(q7_t *base,           // base data
...
static void accumulate_q7_to_q15(q15_t *base, q7_t *target,
...
void arm_maxpool_q7_HWC(q7_t *Im_in, const uint16_t dim_im_in,
                        ...
                        const uint16_t dim_im_out, q7_t *bufferA,
                        q7_t *Im_out) {
  ...
  for (i_y = 0; i_y < dim_im_in; i_y++) {
    ...
    for (i_x = 0; i_x < dim_im_out; i_x++) {
      ...
      if (i_x * stride - padding < 0) {
        ...
        win_start =
            Im_in + (i_y * dim_im_in + i_x * stride - padding) * ch_im_in;
      }
      ...
      if (i_x * stride - padding + dim_kernel >= dim_im_in) {
        ...
        win_stop =
            Im_in +
            (i_y * dim_im_in + i_x * stride - padding + dim_kernel) * ch_im_in;
      }
  ...
  for (i_y = 0; i_y < dim_im_out; i_y++) {
    ...
    if (i_y * stride - padding + dim_kernel >= dim_im_in) {
      ...
      row_end =
          Im_in + (i_y * stride - padding + dim_kernel) * dim_im_in * ch_im_in;
    }
