 ...
 *
 * This function is optimized for convolution with 1x1 kernel size (i.e.,
 * dim_kernel_x=1 and dim_kernel_y=1). It can be used for the second half of
 * MobileNets [1] after depthwise separable convolution.
 *
...
arm_status arm_convolve_1x1_HWC_q7_fast_nonsquare(
  ...
  if (ch_im_in % 4 != 0 || ch_im_out % 2 != 0 || dim_kernel_x != 1 ||
      dim_kernel_y != 1 || padding_x != 0 || padding_y != 0 || stride_x != 1 ||
      stride_y != 1) {
  ...
  for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++) {
    for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++) {
      ...
      arm_q7_to_q15_reordered_no_shift(
          (q7_t *)Im_in + (i_out_y * dim_im_in_x + i_out_x) * ch_im_in, pBuffer,
          ch_im_in);
      pBuffer += ch_im_in;

      if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel_x * dim_kernel_y) {
        pOut = arm_nn_mat_mult_kernel_q7_q15_reordered(wt, bufferA, ch_im_out,
                                                       ch_im_in, bias_shift,
                                                       out_shift, bias, pOut);
        /* counter reset */
        pBuffer = bufferA;
