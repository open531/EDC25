 ...
 * Inner loop: calculate each output value with MAC instruction over an
 * accumulator Mid   loop: loop over different output channel Outer loop: loop
 * over different output (x, y)
 */
      ...
      for (i_ker_y = i_out_y * stride - padding;
        ...
        for (i_ker_x = i_out_x * stride - padding;
          ...
          if (i_ker_y < 0 || i_ker_y >= dim_im_in || i_ker_x < 0 ||
            ...
            /* arm_copy_q7((q7_t *) Im_in + (i_ker_y * dim_im_in + i_ker_x) *
  ...
  for (i_out_y = 0; i_out_y < dim_im_out; i_out_y++) {
    for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++) {
      for (i_ch_out = 0; i_ch_out < ch_im_out; i_ch_out++) {
        // for each output
        conv_out =
            ((q31_t)(bias[i_ch_out]) << bias_shift) + NN_ROUND(out_shift);
        for (i_ker_y = 0; i_ker_y < dim_kernel; i_ker_y++) {
          for (i_ker_x = 0; i_ker_x < dim_kernel; i_ker_x++) {
            ...
            if (in_row >= 0 && in_col >= 0 && in_row < dim_im_in &&
                in_col < dim_im_in) {
              conv_out +=
                  Im_in[(in_row * dim_im_in + in_col) * ch_im_in + i_ch_out] *
