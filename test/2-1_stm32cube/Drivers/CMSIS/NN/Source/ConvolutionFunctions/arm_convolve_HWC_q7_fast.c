...
arm_status arm_convolve_HWC_q7_fast(
  ...
  /*
   *  Here we use bufferA as q15_t internally as computation are done with q15_t
   * level im2col are done to output in q15_t format from q7_t input
   */
  ...
  /*
   *  Here we split the entire matrix into three regions depending on the
   * padding situation Top: i_out_y from 0 to padding - 1 Middle: i_out_y from
   * padding to dim_im_out-padding-1 Bottom: i_out_y from dim_im_out-padding to
   * dim_im_out-1
   */
    ...
    for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++) {
      ...
      for (i_ker_y = i_out_y * stride - padding;
        ...
        for (i_ker_x = i_out_x * stride - padding;
             i_ker_x < i_out_x * stride - padding + dim_kernel; i_ker_x++) {
          if (i_ker_y < 0 || i_ker_y >= dim_im_in || i_ker_x < 0 ||
              i_ker_x >= dim_im_in) {
            /* arm_fill_q15(0, pBuffer, ch_im_in); */
            memset(pBuffer, 0, sizeof(q15_t) * ch_im_in);
          } else {
            arm_q7_to_q15_reordered_no_shift(
                (q7_t *)Im_in + (i_ker_y * dim_im_in + i_ker_x) * ch_im_in,
                pBuffer, ch_im_in);
          }
          pBuffer += ch_im_in;
        }
      ...
      if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel * dim_kernel) {
        pOut = arm_nn_mat_mult_kernel_q7_q15_reordered(
            wt, bufferA, ch_im_out, ch_im_in * dim_kernel * dim_kernel,
...
#else
  /* Run the following code as reference implementation for Cortex-M0 and
   * Cortex-M3 */
