  ...
  for (i_out_y = 0; i_out_y < dim_im_out; i_out_y++) {
    for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++) {
      for (i_ker_y = i_out_y * stride - padding;
        ...
        for (i_ker_x = i_out_x * stride - padding;
          ...
          if (i_ker_y < 0 || i_ker_y >= dim_im_in || i_ker_x < 0 ||
              i_ker_x >= dim_im_in) {
            /* Equivalent to arm_fill_q15(0, pBuffer, ch_im_in) with assumption:
             * ch_im_in = 3 */
            ...
            *(pBuffer + 2) = 0;
             ...
             * (q7_t*)Im_in+(i_ker_y*dim_im_in+i_ker_x)*3, pBuffer, 3);
             */
      ...
      if (pBuffer == bufferA + 2 * 3 * dim_kernel * dim_kernel) {
        pOut = arm_nn_mat_mult_kernel_q7_q15(wt, bufferA, ch_im_out,
