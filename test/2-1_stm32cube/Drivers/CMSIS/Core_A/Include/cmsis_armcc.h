 ...
 */
__STATIC_INLINE uint32_t __get_mode(void) { return (__get_CPSR() & 0x1FU); }

/** \brief  Set Mode
    \param [in]    mode  Mode value to set
 */
__STATIC_INLINE __ASM void __set_mode(uint32_t mode){MOV r1, lr MSR CPSR_C,
                                                     r0 BX r1}

/** \brief  Get Stack Pointer
    \return Stack Pointer
 */
__STATIC_INLINE __ASM uint32_t __get_SP(void){MOV r0, sp BX lr}

/** \brief  Set Stack Pointer
    \param [in]    stack  Stack Pointer value to set
 */
__STATIC_INLINE __ASM void __set_SP(uint32_t stack){MOV sp, r0 BX lr}
...
__STATIC_INLINE __ASM uint32_t __get_SP_usr(void) {
  ARM PRESERVE8
      ...
      MRS R1,
      CPSR CPS #0x1F;
  no effect in USR mode MOV R0, SP MSR CPSR_c, R1;
...
__STATIC_INLINE __ASM void __set_SP_usr(uint32_t topOfProcStack) {
  ARM PRESERVE8
      ...
      MRS R1,
      CPSR CPS #0x1F;
  no effect in USR mode MOV SP, R0 MSR CPSR_c, R1;
  no effect in USR mode ISB BX LR
}
...
#define __get_CP(cp, op1, Rt, CRn, CRm, op2)                                   \
  do {                                                                         \
    register volatile uint32_t tmp __ASM("cp" #cp ":" #op1 ":c" #CRn ":c" #CRm \
                                         ":" #op2);                            \
    (Rt) = tmp;                                                                \
  } while (0)
...
__STATIC_INLINE __ASM void __FPU_Enable(void) {
  ARM
      ...
      MRC p15,
      0, R1, c1, c0, 2 ORR R1, R1, #0x00F00000 MCR p15, 0, R1, c1, c0,
      2
      ...
      ISB
          ...
          // Enable VFP/NEON
          VMRS R1,
      FPEXC ORR R1, R1, #0x40000000 VMSR FPEXC,
      R1
      ...
      R2
          ...
          VMOV D16,
      R2, R2 VMOV D17, R2, R2 VMOV D18, R2, R2 VMOV D19, R2, R2 VMOV D20, R2,
      R2 VMOV D21, R2, R2 VMOV D22, R2, R2 VMOV D23, R2, R2 VMOV D24, R2,
      R2 VMOV D25, R2, R2 VMOV D26, R2, R2 VMOV D27, R2, R2 VMOV D28, R2,
      R2 VMOV D29, R2, R2 VMOV D30, R2, R2 VMOV D31, R2,
      R2 ENDIF

          // Initialise FPSCR to a known state
          VMRS R2,
      FPSCR LDR R3,
      = 0x00086060 // Mask off all bits that do not have to be preserved.
