...
#define JTEST_GROUP_UPDATE_PARENT_GROUP_PF(group_ptr, parent_group_ptr)        \
  do {                                                                         \
    JTEST_GROUP_INC_PASSED(parent_group_ptr, (group_ptr)->passed);             \
    JTEST_GROUP_INC_FAILED(parent_group_ptr, (group_ptr)->failed);             \
  } while (0)
 ...
 */
#define JTEST_GROUP_UPDATE_PARENT_GROUP_OR_FW_PF(group_ptr, parent_group_ptr)  \
  do {                                                                         \
    /* Update the pass fail counts in the parent group */                      \
    if (parent_group_ptr /* Null implies Top*/) {                              \
      JTEST_GROUP_UPDATE_PARENT_GROUP_PF(group_ptr, parent_group_ptr);         \
    } else {                                                                   \
      JTEST_GROUP_UPDATE_FW_PF(group_ptr);                                     \
    }                                                                          \
  } while (0)
...
#define JTEST_GROUP_DUMP_RESULTS(group_ptr)                                    \
  do {                                                                         \
    JTEST_DUMP_STRF("Tests Run: %" PRIu32 "\n"                                 \
                    "----------\n"                                             \
                    "   Passed: %" PRIu32 "\n"                                 \
                    "   Failed: %" PRIu32 "\n",                                \
...
#define JTEST_GROUP_CALL(group_fn)                                             \
  do { /* Save the current group from JTEST_FW_t before swapping */            \
    /* it to this group (in order to restore it later )*/                      \
    JTEST_GROUP_t *__jtest_temp_group_ptr = JTEST_CURRENT_GROUP_PTR();         \
    ...
    JTEST_GROUP_UPDATE_PARENT_GROUP_OR_FW_PF(JTEST_CURRENT_GROUP_PTR(),        \
