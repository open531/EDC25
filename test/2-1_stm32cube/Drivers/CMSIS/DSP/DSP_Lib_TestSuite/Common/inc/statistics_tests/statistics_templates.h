 ...
 */
#define STATISTICS_COMPARE_INTERFACE(block_size, output_type)                  \
  do {                                                                         \
    TEST_ASSERT_BUFFERS_EQUAL(                                                 \
        statistics_output_ref.data_ptr, statistics_output_fut.data_ptr,        \
        1 * sizeof(output_type) /* All fns return one value*/                  \
    );                                                                         \
    TEST_ASSERT_EQUAL(statistics_idx_fut, statistics_idx_ref);                 \
  } while (0)
 ...
 */
#define STATISTICS_SNR_COMPARE_INTERFACE(block_size, output_type)              \
  do {                                                                         \
    TEST_CONVERT_AND_ASSERT_SNR(                                               \
        statistics_output_f32_ref, statistics_output_ref.data_ptr,             \
        statistics_output_f32_fut, statistics_output_fut.data_ptr,             \
        1, /* All fns return one element*/                                     \
        output_type, STATISTICS_SNR_THRESHOLD_##output_type);                  \
  } while (0)
...
#define ARM_max_INPUT_INTERFACE(input, block_size)                             \
  PAREN(input, block_size, statistics_output_fut.data_ptr, &statistics_idx_fut)

#define REF_max_INPUT_INTERFACE(input, block_size)                             \
  PAREN(input, block_size, statistics_output_ref.data_ptr, &statistics_idx_ref)
 ...
 */
#define STATISTICS_DEFINE_TEST_TEMPLATE_BUF1_BLK(                              \
  ...
  JTEST_DEFINE_TEST(arm_##fn_name##_##suffix##_test,                           \
    ...
    TEST_TEMPLATE_BUF1_BLK(                                                    \
        statistics_f_all, statistics_block_sizes, input_type, output_type,     \
