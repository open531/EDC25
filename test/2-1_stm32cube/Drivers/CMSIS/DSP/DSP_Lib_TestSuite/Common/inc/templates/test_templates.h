 ...
 */
#define TEST_CALL_REF(ref, ref_args) TEMPLATE_CALL_FN(ref, ref_args)
...
#define TEST_CONVERT_TO_FLOAT(src_ptr, dst_ptr, block_size, src_type)          \
  do {                                                                         \
    ref_##src_type##_to_float(src_ptr, dst_ptr, block_size);                   \
  } while (0)
...
#define TEST_CONVERT_FLOAT_TO(src_ptr, dst_ptr, block_size, dst_type)          \
  do {                                                                         \
    ref_float_to_##dst_type(src_ptr, dst_ptr, block_size);                     \
  } while (0)
 ...
 */
#define TEST_CONVERT_AND_ASSERT_SNR(ref_dst_ptr, ref_src_ptr, tst_dst_ptr,     \
                                    tst_src_ptr, block_size, tst_src_type,     \
                                    threshold)                                 \
  do {                                                                         \
    TEST_CONVERT_TO_FLOAT(ref_src_ptr, ref_dst_ptr, block_size, tst_src_type); \
    TEST_CONVERT_TO_FLOAT(tst_src_ptr, tst_dst_ptr, block_size, tst_src_type); \
    TEST_ASSERT_SNR(ref_dst_ptr, tst_dst_ptr, block_size, threshold);          \
  } while (0)
 ...
 *  8 * 32 = 256 > 64.
 */
#define TEST_DO_VALID_BLOCKSIZE(block_size, fn_type_spec, input_arr_desc,      \
                                body)                                          \
  do {                                                                         \
    if (block_size * sizeof(fn_type_spec) <= ARR_DESC_BYTES(input_arr_desc)) { \
 ...
 *
 */
#define TEST_TEMPLATE_BUF1_BLK(                                                \
    arr_desc_inputs, arr_desc_block_sizes, input_type, output_type, fut,       \
    fut_arg_interface, ref, ref_arg_interface, compare_interface)              \
  do {                                                                         \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEST_DO_VALID_BLOCKSIZE(                                           \
                ...
                TEST_CALL_FUT_AND_REF(                                         \
                    fut, fut_arg_interface(input_data_ptr, block_size), ref,   \
                    ref_arg_interface(input_data_ptr, block_size));            \
                                                                               \
 ...
 */
#define TEST_TEMPLATE_BUF1_ELT1(arr_desc_inputs, arr_desc_elts, input_type,    \
                                elt_type, output_type, fut, fut_arg_interface, \
                                ref, ref_arg_interface, compare_interface)     \
  do {                                                                         \
    TEMPLATE_DO_ARR_DESC(                                                      \
        input_idx, ARR_DESC_t *, input_ptr, arr_desc_inputs,                   \
        TEMPLATE_DO_ARR_DESC(elt_idx, elt_type, elt, arr_desc_elts,            \
                             void *input_data_ptr = input_ptr->data_ptr;       \
                             TEST_CALL_FUT_AND_REF(                            \
 ...
 *  parameter.
 */
#define TEST_TEMPLATE_BUF1_ELT1_BLK(                                           \
    arr_desc_inputs, arr_desc_elts, arr_desc_block_sizes, input_type,          \
    elt_type, output_type, fut, fut_arg_interface, ref, ref_arg_interface,     \
    compare_interface)                                                         \
  ;                                                                            \
  do {                                                                         \
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEMPLATE_DO_ARR_DESC(                                              \
                ...
                TEST_DO_VALID_BLOCKSIZE(                                       \
                    ...
                    TEST_CALL_FUT_AND_REF(                                     \
                        fut,                                                   \
                        fut_arg_interface(input_data_ptr, elt, block_size),    \
                        ref,                                                   \
                        ref_arg_interface(input_data_ptr, elt, block_size));   \
                    compare_interface(block_size, output_type)))));            \
    return JTEST_TEST_PASSED;                                                  \
  } while (0)

/**
 *  Template for tests that rely on an input buffer, two elements, and a
 * blocksize parameter.
 */
#define TEST_TEMPLATE_BUF1_ELT2_BLK(                                           \
    arr_desc_inputs, arr_desc_elt1s, arr_desc_elt2s, arr_desc_block_sizes,     \
    input_type, elt1_type, elt2_type, output_type, fut, fut_arg_interface,     \
    ref, ref_arg_interface, compare_interface)                                 \
  do {                                                                         \
    TEMPLATE_DO_ARR_DESC(                                                      \
        inut_idx, ARR_DESC_t *, input_ptr, arr_desc_inputs,                    \
            ...
            TEMPLATE_DO_ARR_DESC(                                              \
                ...
                TEMPLATE_DO_ARR_DESC(                                          \
                    ...
                    TEST_DO_VALID_BLOCKSIZE(                                   \
                        ...
                        TEST_CALL_FUT_AND_REF(                                 \
                            fut,                                               \
                            fut_arg_interface(input_data_ptr, elt1, elt2,      \
                                              block_size),                     \
                            ref,                                               \
                            ref_arg_interface(input_data_ptr, elt1, elt2,      \
                                              block_size));                    \
                        compare_interface(block_size, output_type))))));       \
    return JTEST_TEST_PASSED;                                                  \
  } while (0)
 ...
 */
#define TEST_TEMPLATE_BUF2_BLK(arr_desc_inputs_a, arr_desc_inputs_b,           \
                               arr_desc_block_sizes, input_type, output_type,  \
                               fut, fut_arg_interface, ref, ref_arg_interface, \
                               compare_interface)                              \
  do {                                                                         \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            void *input_b_ptr =                                                \
                ARR_DESC_ELT(ARR_DESC_t *, input_idx, &(arr_desc_inputs_b))    \
                    ->data_ptr;                                                \
                                                                               \
            TEST_DO_VALID_BLOCKSIZE(                                           \
                block_size, input_type, input_ptr,                             \
                TEST_CALL_FUT_AND_REF(                                         \
                    fut,                                                       \
                    fut_arg_interface(input_a_ptr, input_b_ptr, block_size),   \
                    ref,                                                       \
                    ref_arg_interface(input_a_ptr, input_b_ptr, block_size));  \
                                                                               \
                compare_interface(block_size, output_type))));                 \
    return JTEST_TEST_PASSED;                                                  \
  } while (0)
 ...
 */
#define TEST_TEMPLATE_ELT1_BLK(arr_desc_elts, arr_desc_block_sizes, elt_type,  \
                               output_type, fut, fut_arg_interface, ref,       \
                               ref_arg_interface, compare_interface)           \
  do {                                                                         \
    TEMPLATE_DO_ARR_DESC(                                                      \
        block_size_idx, uint32_t, block_size, arr_desc_block_sizes,            \
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEST_CALL_FUT_AND_REF(fut, fut_arg_interface(elt, block_size),     \
                                  ref, ref_arg_interface(elt, block_size));    \
            compare_interface(block_size, output_type)));                      \
    return JTEST_TEST_PASSED;                                                  \
  } while (0)
