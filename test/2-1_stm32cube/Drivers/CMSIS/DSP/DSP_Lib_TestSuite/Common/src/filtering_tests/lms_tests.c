  ...
  JTEST_DEFINE_TEST(arm_lms##config_suffix##_##suffix##_test,                  \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            arm_fir_init_##suffix(&fir_inst, numTaps,                          \
                                  (output_type *)filtering_coeffs_##suffix,    \
                                  (void *)filtering_pState, blockSize);        \
                                                                               \
            ref_fir_##suffix(&fir_inst, (void *)filtering_##suffix##_inputs,   \
                             (void *)filtering_input_lms, blockSize);          \
                                                                               \
            for (i = 0; i < numTaps; i++) {                                    \
            ...
            arm_lms##config_suffix##_init_##suffix(                            \
                &lms_inst_fut, numTaps, (output_type *)filtering_coeffs_lms,   \
                (void *)filtering_pState, mu, blockSize);                      \
                                                                               \
            JTEST_COUNT_CYCLES(arm_lms##config_suffix##_##suffix(              \
                &lms_inst_fut, (void *)filtering_output_f32_fut,               \
                (void *)filtering_input_lms, (void *)filtering_output_fut,     \
            ...
            for (i = 0; i < numTaps; i++) {                                    \
              *((output_type *)filtering_coeffs_lms + i) = (output_type)0;     \
                                                                               ...
                                                                               \
            arm_lms##config_suffix##_init_##suffix(                            \
                &lms_inst_ref, numTaps, (output_type *)filtering_coeffs_lms,   \
                (void *)filtering_pState, mu, blockSize);                      \
                                                                               \
            ref_lms##config_suffix##_##suffix(                                 \
                &lms_inst_ref, (void *)filtering_output_f32_fut,               \
                (void *)filtering_input_lms, (void *)filtering_output_ref,     \
                (void *)((output_type *)filtering_output_fut + blockSize),     \
                blockSize);                                                    \
                                                                               \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEMPLATE_DO_ARR_DESC(                                              \
                ...
                filtering_postshifts, /* Initialize the FIR Instances */       \
                arm_fir_init_##suffix(                                         \
                    &fir_inst, numTaps,                                        \
                    (output_type *)filtering_coeffs_##suffix,                  \
                    (void *)filtering_pState, blockSize);                      \
                                                                               \
                ref_fir_##suffix(&fir_inst,                                    \
                ...
                JTEST_DUMP_STRF("Block Size: %d\n"                             \
                                "Number of Taps: %d\n"                         \
                                "Post Shift: %d\n",                            \
                                (int)blockSize, (int)numTaps, (int)postShift); \
                                                                               \
                /* Initialize the LMS Instances */                             \
                arm_lms##config_suffix##_init_##suffix(                        \
                    &lms_inst_fut, numTaps,                                    \
                    (output_type *)filtering_coeffs_lms,                       \
                    (void *)filtering_pState, mu_##suffix, blockSize,          \
                    postShift);                                                \
                                                                               \
                JTEST_COUNT_CYCLES(arm_lms##config_suffix##_##suffix(          \
                    &lms_inst_fut, (void *)filtering_output_f32_fut,           \
