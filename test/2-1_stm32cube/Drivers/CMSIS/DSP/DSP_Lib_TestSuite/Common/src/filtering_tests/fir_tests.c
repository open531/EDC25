...
#define FIR_DEFINE_TEST(suffix, config_suffix, output_type)                    \
  JTEST_DEFINE_TEST(arm_fir##config_suffix##_##suffix##_test,                  \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            numtaps_idx, uint16_t, numTaps,                                    \
            filtering_numtaps, /* Initialize the FIR Instances */              \
            arm_fir_init_##suffix(&fir_inst_fut, numTaps,                      \
            ...
            JTEST_DUMP_STRF("Block Size: %d\n"                                 \
                            "Number of Taps: %d\n",                            \
                            (int)blockSize, (int)numTaps);                     \
                                                                               \
            JTEST_COUNT_CYCLES(arm_fir##config_suffix##_##suffix(              \
                &fir_inst_fut, (void *)filtering_##suffix##_inputs,            \
                (void *)filtering_output_fut, blockSize));                     \
                                                                               \
            arm_fir_init_##suffix(&fir_inst_ref, numTaps,                      \
                                  (output_type *)filtering_coeffs_##suffix,    \
                                  (void *)filtering_pState, blockSize);        \
                                                                               \
            ref_fir##config_suffix##_##suffix(                                 \
                &fir_inst_ref, (void *)filtering_##suffix##_inputs,            \
                (void *)filtering_output_ref, blockSize);                      \
                                                                               \
            FILTERING_SNR_COMPARE_INTERFACE(blockSize, output_type)));         \
                                                                               \
  ...
  JTEST_DEFINE_TEST(arm_fir_interpolate_##suffix##_test,                       \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEMPLATE_DO_ARR_DESC(                                              \
                L_idx, uint8_t, L,                                             \
                filtering_Ls, /* Display test parameter values */              \
                JTEST_DUMP_STRF("Block Size: %d\n"                             \
                                "Number of Taps: %d\n"                         \
                                "Upsample factor: %d\n",                       \
                                (int)blockSize, (int)numTaps, (int)L);         \
                                                                               \
                ...
                arm_fir_interpolate_init_##suffix(                             \
                    ...
                    (void *)filtering_pState, blockSize);                      \
                                                                               \
                JTEST_COUNT_CYCLES(arm_fir_interpolate_##suffix(               \
                    &fir_inst_fut, (void *)filtering_##suffix##_inputs,        \
                    (void *)filtering_output_fut, blockSize));                 \
                                                                               \
                arm_fir_interpolate_init_##suffix(                             \
                ...
                ref_fir_interpolate_##suffix(                                  \
                    ...
                    (void *)filtering_output_ref, blockSize);                  \
                                                                               \
                FILTERING_SNR_COMPARE_INTERFACE(blockSize * (uint32_t)L,       \
                                                output_type))));               \
                                                                               \
    return JTEST_TEST_PASSED;                                                  \
  }
  ...
  JTEST_DEFINE_TEST(arm_fir_decimate##config_suffix##_##suffix##_test,         \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            TEMPLATE_DO_ARR_DESC(                                              \
                M_idx, uint8_t, M, filtering_Ms, if (blockSize % M == 0) {     \
                  /* Display test parameter values */                          \
                  JTEST_DUMP_STRF("Block Size: %d\n"                           \
                                  "Number of Taps: %d\n"                       \
                                  "Decimation Factor: %d\n",                   \
                                  (int)blockSize, (int)numTaps, (int)M);       \
                                                                               \
                  /* Initialize the FIR Instances */                           \
                  ...
                  ref_fir_decimate##config_suffix##_##suffix(                  \
                      ...
                      (void *)filtering_output_ref, blockSize);                \
                                                                               \
                  FILTERING_SNR_COMPARE_INTERFACE(blockSize / M, output_type); \
                })));                                                          \
                                                                               \
    return JTEST_TEST_PASSED;                                                  \
  }
...
#define FIR_LATTICE_DEFINE_TEST(suffix, output_type)                           \
  JTEST_DEFINE_TEST(arm_fir_lattice_##suffix##_test,                           \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        blocksize_idx, uint32_t, blockSize, filtering_blocksizes,              \
        TEMPLATE_DO_ARR_DESC(                                                  \
            numstages_idx, uint16_t, numStages,                                \
            filtering_numstages, /* Display test parameter values */           \
            JTEST_DUMP_STRF("Block Size: %d\n"                                 \
                            "Number of Stages: %d\n",                          \
                            (int)blockSize, (int)numStages);                   \
                                                                               \
            /* Initialize the FIR Instances */                                 \
            arm_fir_lattice_init_##suffix(                                     \
                ...
                (void *)filtering_pState);                                     \
                                                                               \
            JTEST_COUNT_CYCLES(arm_fir_lattice_##suffix(                       \
                &fir_inst_fut, (void *)filtering_##suffix##_inputs,            \
                (void *)filtering_output_fut, blockSize));                     \
                                                                               \
            arm_fir_lattice_init_##suffix(                                     \
                ...
                (void *)filtering_pState);                                     \
                                                                               \
            ref_fir_lattice_##suffix(&fir_inst_ref,                            \
...
#define FIR_SPARSE_DEFINE_TEST(suffix, output_type)                            \
  JTEST_DEFINE_TEST(arm_fir_sparse_##suffix##_test, arm_fir_sparse_##suffix) { \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            numtaps_idx, uint16_t, numTaps,                                    \
            filtering_numtaps, /* Display test parameter values */             \
            JTEST_DUMP_STRF("Block Size: %d\n"                                 \
                            "Number of Taps: %d\n"                             \
                            "Tap Delay: %d\n",                                 \
                            (int)blockSize, (int)numTaps,                      \
            ...
            arm_fir_sparse_init_##suffix(                                      \
                ...
                (output_type *)filtering_coeffs_##suffix,                      \
                (void *)filtering_pState, (int32_t *)filtering_tap_delay,      \
                FILTERING_MAX_TAP_DELAY, blockSize);                           \
                                                                               \
            JTEST_COUNT_CYCLES(arm_fir_sparse_##suffix(                        \
                &fir_inst_fut, (void *)filtering_##suffix##_inputs,            \
            ...
            arm_fir_sparse_init_##suffix(                                      \
                ...
                FILTERING_MAX_TAP_DELAY, blockSize);                           \
                                                                               \
            ref_fir_sparse_##suffix(&fir_inst_ref,                             \
                                    (void *)filtering_##suffix##_inputs,       \
                                    (void *)filtering_output_ref,              \
                                    (void *)filtering_scratch, blockSize);     \
                                                                               \
            FILTERING_SNR_COMPARE_INTERFACE(blockSize, output_type)));         \
                                                                               \
    return JTEST_TEST_PASSED;                                                  \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TEMPLATE_DO_ARR_DESC(                                                  \
            ...
            JTEST_COUNT_CYCLES(arm_fir_sparse_##suffix(                        \
                &fir_inst_fut, (void *)filtering_##suffix##_inputs,            \
                (void *)filtering_output_fut, (void *)filtering_scratch,       \
                (void *)filtering_scratch2, blockSize));                       \
                                                                               \
            arm_fir_sparse_init_##suffix(                                      \
                &fir_inst_ref, numTaps,                                        \
                (output_type *)filtering_coeffs_##suffix,                      \
