  ...
  do {                                                                         \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        JTEST_DUMP_STRF("Block Size: %d\n"                                     \
                        ...
                        (int)cfft_inst_ptr->fftLen, (int)ifft_flag);           \
                                                                               \
        /* Display cycle count and run test */                                 \
        JTEST_COUNT_CYCLES(arm_cfft_##suffix(                                  \
            cfft_inst_ptr, (void *)transform_fft_inplace_input_fut,            \
            ifft_flag, /* IFFT Flag */                                         \
            1));       /* Bitreverse flag */                                   \
        ref_cfft_##suffix(cfft_inst_ptr,                                       \
                          ...
                          1);        /* Bitreverse flag */                     \
                                                                               \
        /* Test correctness */                                                 \
        TRANSFORM_SNR_COMPARE_CMPLX_INTERFACE(cfft_inst_ptr->fftLen,           \
                                              output_type));                   \
                                                                               \
...
/*
  CFFT function with downshift test template. Arguments are: inverse-transform
  flag, function suffix (q7/q15/q31/f32) and the output type (q7_t, q15_t,
  q31_t, float32_t)
*/
  ...
  do {                                                                         \
    ...
    TEMPLATE_DO_ARR_DESC(                                                      \
        ...
        TRANSFORM_PREPARE_INPLACE_INPUTS_DOWNSHIFT(                            \
            ...
            cfft_inst_ptr->fftLen * sizeof(output_type) *                      \
                2 /*complex_inputs*/,                                          \
            output_type);                                                      \
                                                                               \
        /* Display parameter values */                                         \
