...
#define ARM_PID_TEST(suffix, type)                                             \
  JTEST_DEFINE_TEST(arm_pid_##suffix##_test, arm_pid_##suffix) {               \
    ...
    for (i = 0; i < CONTROLLER_MAX_COEFFS_LEN / 3; i++) {                      \
      ...
      JTEST_COUNT_CYCLES(for (j = 0; j < CONTROLLER_MAX_LEN; j++) {            \
        *((type *)controller_output_fut + j) =                                 \
            arm_pid_##suffix(&fut_pid_inst, controller_##suffix##_inputs[j]);  \
      });                                                                      \
                                                                               \
      for (j = 0; j < CONTROLLER_MAX_LEN; j++) {                               \
        *((type *)controller_output_ref + j) =                                 \
            ref_pid_##suffix(&ref_pid_inst, controller_##suffix##_inputs[j]);  \
      }                                                                        \
                                                                               \
      /* Test correctness */                                                   \
      CONTROLLER_SNR_COMPARE_INTERFACE(CONTROLLER_MAX_LEN, type);              \
    }                                                                          \
                                                                               \
