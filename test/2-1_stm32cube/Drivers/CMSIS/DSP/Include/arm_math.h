...
/**
   ...
   * a suite of common signal processing functions for use on Cortex-M processor
   based devices.
   *
   * The library is divided into a number of functions each covering a specific
   category:
   ...
   *
   * The library has separate functions for operating on 8-bit integers, 16-bit
   integers,
   ...
   * - arm_cortexM7lfdp_math.lib (Cortex-M7, Little endian, Double Precision
   Floating Point Unit)
   * - arm_cortexM7bfdp_math.lib (Cortex-M7, Big endian, Double Precision
   Floating Point Unit)
   * - arm_cortexM7lfsp_math.lib (Cortex-M7, Little endian, Single Precision
   Floating Point Unit)
   * - arm_cortexM7bfsp_math.lib (Cortex-M7, Big endian and Single Precision
   ...
   * - arm_ARMv8MMLlfsp_math.lib (Armv8-M Mainline, Little endian, Single
   Precision Floating Point Unit)
   * - arm_ARMv8MMLld_math.lib (Armv8-M Mainline, Little endian, DSP
   instructions)
   * - arm_ARMv8MMLldfsp_math.lib (Armv8-M Mainline, Little endian, DSP
   instructions, Single Precision Floating Point Unit)
   *
   * The library functions are declared in the public file
   <code>arm_math.h</code> which is placed in the <code>Include</code> folder.
   * Simply include this file and link the appropriate library in the
   application and begin calling the library functions. The Library supports
   single
   * public header file <code> arm_math.h</code> for Cortex-M cores with little
   endian and big endian. Same header file will be used for floating point
   unit(FPU) variants.
   * Define the appropriate preprocessor macro ARM_MATH_CM7 or ARM_MATH_CM4 or
   ARM_MATH_CM3 or
   * ARM_MATH_CM0 or ARM_MATH_CM0PLUS depending on the target processor in the
   application.
   * For Armv8-M cores define preprocessor macro ARM_MATH_ARMV8MBL or
   ARM_MATH_ARMV8MML.
   * Set preprocessor macro __DSP_PRESENT if Armv8-M Mainline core supports DSP
   instructions.
   *
   *
   * Examples
   * --------
   *
   * The library ships with a number of examples which demonstrate how to use
   ...
   * ------------
   *
   * The library installer contains a project file to rebuild libraries on MDK
   toolchain in the <code>CMSIS\\DSP_Lib\\Source\\ARM</code> folder.
   * - arm_cortexM_math.uvprojx
   *
   *
   * The libraries can be built by opening the arm_cortexM_math.uvprojx project
   in MDK-ARM, selecting a specific target, and defining the optional
   preprocessor macros detailed above.
   *
   * Preprocessor Macros
   * ------------
   *
   ...
   *
   * Define macro UNALIGNED_SUPPORT_DISABLE, If the silicon does not support
   unaligned memory access
   *
   * - ARM_MATH_BIG_ENDIAN:
   *
   * Define macro ARM_MATH_BIG_ENDIAN to build the library for big endian
   targets. By default library builds for little endian targets.
   *
   * - ARM_MATH_MATRIX_CHECK:
   *
   * Define macro ARM_MATH_MATRIX_CHECK for checking on the input and output
   sizes of matrices
   *
   * - ARM_MATH_ROUNDING:
   *
   ...
   * and ARM_MATH_CM0 for building library on Cortex-M0 target, ARM_MATH_CM0PLUS
   for building library on Cortex-M0+ target, and
   * ARM_MATH_CM7 for building the library on cortex-M7.
   *
   * - ARM_MATH_ARMV8MxL:
   *
   * Define macro ARM_MATH_ARMV8MBL for building the library on Armv8-M Baseline
   target, ARM_MATH_ARMV8MML for building library
   * on Armv8-M Mainline target.
   *
   * - __FPU_PRESENT:
   *
   * Initialize macro __FPU_PRESENT = 1 when building on FPU supported Targets.
   Enable this macro for floating point libraries.
   *
   * - __DSP_PRESENT:
   *
   * Initialize macro __DSP_PRESENT = 1 when Armv8-M Mainline core supports DSP
   instructions.
   *
   * <hr>
   ...
   * -----------------------------
   *
   * The following files relevant to CMSIS-DSP are present in the
   <b>ARM::CMSIS</b> Pack directories:
   * |File/Folder                   |Content |
   * |------------------------------|------------------------------------------------------------------------|
   * |\b CMSIS\\Documentation\\DSP  | This documentation |
   * |\b CMSIS\\DSP_Lib             | Software license agreement (license.txt) |
   * |\b CMSIS\\DSP_Lib\\Examples   | Example projects demonstrating the usage
 ...
 * library, the fast math functions operate on individual values and not arrays.
 ...
 * Refer to the function <code>arm_mat_init_f32()</code>,
 * <code>arm_mat_init_q31()</code> and <code>arm_mat_init_q15()</code> for
 * floating-point, Q31 and Q15 types,  respectively.
 *
 * \par
 ...
 * data section. To place the instance structure in a const data section,
 * manually initialize the data structure.  For example: <pre>
...
typedef enum {
  ...
  ARM_MATH_SINGULAR = -5, /**< Generated by matrix inversion if the input matrix
...
CMSIS_INLINE __STATIC_INLINE q7_t clip_q31_to_q7(q31_t x) {
  return ((q31_t)(x >> 24) != ((q31_t)x >> 23)) ? ((0x7F ^ ((q7_t)(x >> 31))))
                                                : (q7_t)x;
}
...
CMSIS_INLINE __STATIC_INLINE uint32_t arm_recip_q31(q31_t in, q31_t *dst,
                                                    q31_t *pRecipTable) {
  q31_t out;
  uint32_t tempVal;
  uint32_t index, i;
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint32_t arm_recip_q15(q15_t in, q15_t *dst,
                                                    q15_t *pRecipTable) {
  q15_t out = 0;
  uint32_t tempVal = 0;
...
CMSIS_INLINE __STATIC_INLINE uint32_t __QADD8(uint32_t x, uint32_t y) {
  ...
  r = __SSAT(((((q31_t)x << 24) >> 24) + (((q31_t)y << 24) >> 24)), 8) &
      (int32_t)0x000000FF;
  s = __SSAT(((((q31_t)x << 16) >> 24) + (((q31_t)y << 16) >> 24)), 8) &
      (int32_t)0x000000FF;
  t = __SSAT(((((q31_t)x << 8) >> 24) + (((q31_t)y << 8) >> 24)), 8) &
      (int32_t)0x000000FF;
  u = __SSAT(((((q31_t)x) >> 24) + (((q31_t)y) >> 24)), 8) &
      (int32_t)0x000000FF;
...
CMSIS_INLINE __STATIC_INLINE uint32_t __QADD16(uint32_t x, uint32_t y) {
  /*  q31_t r,     s;  without initialisation 'arm_offset_q15 test' fails  but
   * 'intrinsic' tests pass! for armCC */
  q31_t r = 0, s = 0;

  r = __SSAT(((((q31_t)x << 16) >> 16) + (((q31_t)y << 16) >> 16)), 16) &
      (int32_t)0x0000FFFF;
...
CMSIS_INLINE __STATIC_INLINE uint32_t __QASX(uint32_t x, uint32_t y) {
  ...
  r = __SSAT(((((q31_t)x << 16) >> 16) - (((q31_t)y) >> 16)), 16) &
      (int32_t)0x0000FFFF;
  s = __SSAT(((((q31_t)x) >> 16) + (((q31_t)y << 16) >> 16)), 16) &
      (int32_t)0x0000FFFF;
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint32_t __SMLAD(uint32_t x, uint32_t y,
                                              uint32_t sum) {
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint32_t __SMLADX(uint32_t x, uint32_t y,
                                               uint32_t sum) {
  return ((uint32_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y) >> 16)) +
                     ((((q31_t)x) >> 16) * (((q31_t)y << 16) >> 16)) +
                     (((q31_t)sum))));
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint32_t __SMLSDX(uint32_t x, uint32_t y,
                                               uint32_t sum) {
  return ((uint32_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y) >> 16)) -
                     ((((q31_t)x) >> 16) * (((q31_t)y << 16) >> 16)) +
                     (((q31_t)sum))));
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint64_t __SMLALD(uint32_t x, uint32_t y,
                                               uint64_t sum) {
  /*  return (sum + ((q15_t) (x >> 16) * (q15_t) (y >> 16)) + ((q15_t) x *
   * (q15_t) y)); */
 ...
 */
CMSIS_INLINE __STATIC_INLINE uint64_t __SMLALDX(uint32_t x, uint32_t y,
                                                uint64_t sum) {
  /*  return (sum + ((q15_t) (x >> 16) * (q15_t) y)) + ((q15_t) x * (q15_t) (y
   * >> 16)); */
 ...
 */
CMSIS_INLINE __STATIC_INLINE int32_t __SMMLA(int32_t x, int32_t y,
                                             int32_t sum) {
...
typedef struct {
  ...
  q7_t *pState;  /**< points to the state variable array. The array is of length
                    numTaps+blockSize-1. */
  q7_t *pCoeffs; /**< points to the coefficient array. The array is of length
                    numTaps.*/
} arm_fir_instance_q7;
...
typedef struct {
  uint16_t numTaps; /**< number of filter coefficients in the filter. */
  q15_t *pState; /**< points to the state variable array. The array is of length
                    numTaps+blockSize-1. */
  q15_t *pCoeffs; /**< points to the coefficient array. The array is of length
                     numTaps.*/
...
typedef struct {
  uint16_t numTaps; /**< number of filter coefficients in the filter. */
  q31_t *pState; /**< points to the state variable array. The array is of length
                    numTaps+blockSize-1. */
  q31_t *pCoeffs; /**< points to the coefficient array. The array is of length
                     numTaps. */
...
typedef struct {
  uint16_t numTaps;   /**< number of filter coefficients in the filter. */
  float32_t *pState;  /**< points to the state variable array. The array is of
                         length numTaps+blockSize-1. */
  float32_t *pCoeffs; /**< points to the coefficient array. The array is of
                         length numTaps. */
 ...
 */
void arm_fir_init_q7(arm_fir_instance_q7 *S, uint16_t numTaps, q7_t *pCoeffs,
...
/**
 * @brief Processing function for the fast Q15 FIR filter for Cortex-M3 and
 * Cortex-M4.
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_fir_fast_q15(const arm_fir_instance_q15 *S, q15_t *pSrc, q15_t *pDst,
                      uint32_t blockSize);
 ...
 * structure.
 * @param[in]     numTaps    Number of filter coefficients in the filter. Must
 * be even and greater than or equal to 4.
 * @param[in]     pCoeffs    points to the filter coefficients.
 * @param[in]     pState     points to the state buffer.
 * @param[in]     blockSize  number of samples that are processed at a time.
 * @return The function returns ARM_MATH_SUCCESS if initialization was
 * successful or ARM_MATH_ARGUMENT_ERROR if <code>numTaps</code> is not a
 * supported value.
 */
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_fir_q31(const arm_fir_instance_q31 *S, q31_t *pSrc, q31_t *pDst,
                 uint32_t blockSize);

/**
 * @brief Processing function for the fast Q31 FIR filter for Cortex-M3 and
 * Cortex-M4.
 ...
 * @param[in]     blockSize  number of samples that are processed at a time.
 */
void arm_fir_init_q31(arm_fir_instance_q31 *S, uint16_t numTaps, q31_t *pCoeffs,
 ...
 */
typedef struct {
  int8_t numStages; /**< number of 2nd order stages in the filter.  Overall
                       order is 2*numStages. */
  q15_t *pState;  /**< Points to the array of state coefficients.  The array is
                     of length 4*numStages. */
  q15_t *pCoeffs; /**< Points to the array of coefficients.  The array is of
                     length 5*numStages. */
  int8_t postShift; /**< Additional shift, in bits, applied to each output
                       sample. */
...
typedef struct {
  uint32_t numStages; /**< number of 2nd order stages in the filter.  Overall
                         order is 2*numStages. */
  q31_t *pState;  /**< Points to the array of state coefficients.  The array is
                     of length 4*numStages. */
  q31_t *pCoeffs; /**< Points to the array of coefficients.  The array is of
                     length 5*numStages. */
  uint8_t postShift; /**< Additional shift, in bits, applied to each output
                        sample. */
 ...
 * @param[in]     pCoeffs    points to the filter coefficients.
 * @param[in]     pState     points to the state buffer.
 * @param[in]     postShift  Shift to be applied to the output. Varies according
 * to the coefficients format
 */
void arm_biquad_cascade_df1_init_q15(arm_biquad_casd_df1_inst_q15 *S,
                                     uint8_t numStages, q15_t *pCoeffs,
                                     q15_t *pState, int8_t postShift);

/**
 * @brief Fast but less precise processing function for the Q15 Biquad cascade
...
/**
 * @brief Fast but less precise processing function for the Q31 Biquad cascade
 * filter for Cortex-M3 and Cortex-M4.
 * @param[in]  S          points to an instance of the Q31 Biquad cascade
 * structure.
 * @param[in]  pSrc       points to the block of input data.
 ...
 * @param[in,out] S               points to an instance of the Q15 PID
 * structure.
 * @param[in]     resetStateFlag  flag to reset the state. 0 = no change in
 * state 1 = reset the state.
 */
...
/**
 * @brief Instance structure for the floating-point bilinear interpolation
 * function.
 */
typedef struct {
...
typedef struct {
  ...
  uint8_t ifftFlag;       /**< flag that selects forward (ifftFlag=0) or inverse
                             (ifftFlag=1) transform. */
  uint8_t bitReverseFlag; /**< flag that enables (bitReverseFlag=1) or disables
                             (bitReverseFlag=0) bit reversal of output. */
  q15_t *pTwiddle;        /**< points to the Sin twiddle factor table. */
  uint16_t *pBitRevTable; /**< points to the bit reversal table. */
  uint16_t twidCoefModifier; /**< twiddle coefficient modifier that supports
                                different size FFTs with the same twiddle factor
                                table. */
  uint16_t bitRevFactor; /**< bit reversal modifier that supports different size
...
typedef struct {
  ...
  uint8_t ifftFlagR;   /**< flag that selects forward (ifftFlagR=0) or inverse
                          (ifftFlagR=1) transform. */
  uint8_t
      bitReverseFlagR; /**< flag that enables (bitReverseFlagR=1) or disables
                          (bitReverseFlagR=0) bit reversal of output. */
  uint32_t twidCoefRModifier; /**< twiddle coefficient modifier that supports
 ...
 * is successful or ARM_MATH_ARGUMENT_ERROR if <code>fftLenReal</code> is not a
 ...
 * @param[in]     normalize  normalizing factor.
 * @return      arm_status function returns ARM_MATH_SUCCESS if initialization
 * is successful or ARM_MATH_ARGUMENT_ERROR if <code>N</code> is not a supported
 * transform length.
 */
 ...
 * @param[in]     S              points to an instance of the Q31 DCT4
 * structure.
 * @param[in]     pState         points to state buffer.
 * @param[in,out] pInlineBuffer  points to the in-place input and output buffer.
 */
 ...
 * @param[in]     S              points to an instance of the Q15 DCT4
 * structure.
 * @param[in]     pState         points to state buffer.
 * @param[in,out] pInlineBuffer  points to the in-place input and output buffer.
 */
 ...
 * @param[in]  blockSize   number of samples in the vector
 */
void arm_scale_q7(q7_t *pSrc, q7_t scaleFract, int8_t shift, q7_t *pDst,
                  uint32_t blockSize);
 ...
 * @param[in]  blockSize   number of samples in the vector
 */
void arm_scale_q31(q31_t *pSrc, q31_t scaleFract, int8_t shift, q31_t *pDst,
                   uint32_t blockSize);
 ...
 * @param[out] result     output result returned here
 */
void arm_dot_prod_q15(q15_t *pSrcA, q15_t *pSrcB, uint32_t blockSize,
                      q63_t *result);
 ...
 * @param[out] result     output result returned here
 */
void arm_dot_prod_q31(q31_t *pSrcA, q31_t *pSrcB, uint32_t blockSize,
                      q63_t *result);
 ...
 * @param[in]  blockSize  number of samples in the vector
 */
void arm_shift_q31(q31_t *pSrc, int8_t shiftBits, q31_t *pDst,
                   uint32_t blockSize);
 ...
 * @param[in]  blockSize  number of samples in the vector
 */
void arm_offset_f32(float32_t *pSrc, float32_t offset, float32_t *pDst,
                    uint32_t blockSize);
 ...
 * @param[in]  pSrcB      points to the second input sequence.
 * @param[in]  srcBLen    length of the second input sequence.
 * @param[out] pDst       points to the block of output data  Length
 * srcALen+srcBLen-1.
 ...
 * srcBLen).
 */
void arm_conv_opt_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
 ...
 * written.  Length srcALen+srcBLen-1.
 */
void arm_conv_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
 ...
 * srcALen+srcBLen-1.
 */
void arm_conv_fast_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
                       uint32_t srcBLen, q15_t *pDst);
 ...
 * srcALen+srcBLen-1.
 */
void arm_conv_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
                  uint32_t srcBLen, q31_t *pDst);

/**
 * @brief Convolution of Q31 sequences (fast version) for Cortex-M3 and
 * Cortex-M4
 * @param[in]  pSrcA    points to the first input sequence.
 * @param[in]  srcALen  length of the first input sequence.
 * @param[in]  pSrcB    points to the second input sequence.
 ...
 * srcALen+srcBLen-1.
 */
void arm_conv_fast_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
                       uint32_t srcBLen, q31_t *pDst);
 ...
 * min(srcALen, srcBLen).
 */
void arm_conv_opt_q7(q7_t *pSrcA, uint32_t srcALen, q7_t *pSrcB,
 ...
 * srcALen+srcBLen-1.
 */
void arm_conv_q7(q7_t *pSrcA, uint32_t srcALen, q7_t *pSrcB, uint32_t srcBLen,
                 q7_t *pDst);
 ...
 * @return  Returns either ARM_MATH_SUCCESS if the function completed correctly
 * or ARM_MATH_ARGUMENT_ERROR if the requested subset is not in the range [0
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_f32(float32_t *pSrcA, uint32_t srcALen,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_opt_q15(q15_t *pSrcA, uint32_t srcALen,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
                                uint32_t srcBLen, q15_t *pDst,
                                uint32_t firstIndex, uint32_t numPoints);

/**
 * @brief Partial convolution of Q15 sequences (fast version) for Cortex-M3 and
 * Cortex-M4
 * @param[in]  pSrcA       points to the first input sequence.
 * @param[in]  srcALen     length of the first input sequence.
 * @param[in]  pSrcB       points to the second input sequence.
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_fast_q15(q15_t *pSrcA, uint32_t srcALen,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_fast_opt_q15(q15_t *pSrcA, uint32_t srcALen,
                                         q15_t *pSrcB, uint32_t srcBLen,
                                         q15_t *pDst, uint32_t firstIndex,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
                                uint32_t srcBLen, q31_t *pDst,
                                uint32_t firstIndex, uint32_t numPoints);

/**
 * @brief Partial convolution of Q31 sequences (fast version) for Cortex-M3 and
 * Cortex-M4
 * @param[in]  pSrcA       points to the first input sequence.
 * @param[in]  srcALen     length of the first input sequence.
 * @param[in]  pSrcB       points to the second input sequence.
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_fast_q31(q31_t *pSrcA, uint32_t srcALen,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_opt_q7(q7_t *pSrcA, uint32_t srcALen, q7_t *pSrcB,
 ...
 * srcALen+srcBLen-2].
 */
arm_status arm_conv_partial_q7(q7_t *pSrcA, uint32_t srcALen, q7_t *pSrcB,
                               uint32_t srcBLen, q7_t *pDst,
                               uint32_t firstIndex, uint32_t numPoints);
 ...
 * successful or ARM_MATH_LENGTH_ERROR if <code>blockSize</code> is not a
 * multiple of <code>M</code>.
 */
...
/**
 * @brief Processing function for the Q15 FIR decimator (fast variant) for
 ...
 * @param[in] blockSize number of input samples to process per call.
 */
void arm_fir_decimate_q31(const arm_fir_decimate_instance_q31 *S, q31_t *pSrc,
                          q31_t *pDst, uint32_t blockSize);

/**
 * @brief Processing function for the Q31 FIR decimator (fast variant) for
 * Cortex-M3 and Cortex-M4.
 ...
 * @param[in]  blockSize  number of input samples to process per call.
 */
void arm_fir_decimate_fast_q31(arm_fir_decimate_instance_q31 *S, q31_t *pSrc,
                               q31_t *pDst, uint32_t blockSize);
...
typedef struct {
  ...
  uint16_t phaseLength; /**< length of each polyphase filter component. */
  float32_t *pCoeffs;   /**< points to the coefficient array. The array is of
                           length L*phaseLength. */
  float32_t *pState;    /**< points to the state variable array. The array is of
                           length phaseLength+numTaps-1. */
 ...
 * @param[in]     blockSize  number of input samples to process per call.
 * @return        The function returns ARM_MATH_SUCCESS if initialization is
 * successful or ARM_MATH_LENGTH_ERROR if the filter length <code>numTaps</code>
 * is not a multiple of the interpolation factor <code>L</code>.
 */
...
typedef struct {
  uint8_t numStages; /**< number of 2nd order stages in the filter.  Overall
                        order is 2*numStages. */
  q63_t *pState;  /**< points to the array of state coefficients.  The array is
                     of length 4*numStages. */
  q31_t *pCoeffs; /**< points to the array of coefficients.  The array is of
                     length 5*numStages. */
  uint8_t postShift; /**< additional shift, in bits, applied to each output
...
/**
 * @param[in]  S          points to an instance of the high precision Q31 Biquad
...
typedef struct {
  uint8_t numStages;  /**< number of 2nd order stages in the filter.  Overall
                         order is 2*numStages. */
  float64_t *pState;  /**< points to the array of state coefficients.  The array
                         is of length 2*numStages. */
  float64_t *pCoeffs; /**< points to the array of coefficients.  The array is of
                         length 5*numStages. */
} arm_biquad_cascade_df2T_instance_f64;

/**
 * @brief Processing function for the floating-point transposed direct form II
 * Biquad cascade filter.
...
/**
 * @brief Processing function for the floating-point transposed direct form II
 * Biquad cascade filter. 2 channels
 * @param[in]  S          points to an instance of the filter data structure.
 * @param[in]  pSrc       points to the block of input data.
 * @param[out] pDst       points to the block of output data
 * @param[in]  blockSize  number of samples to process.
...
void arm_biquad_cascade_stereo_df2T_f32(
    const arm_biquad_cascade_stereo_df2T_instance_f32 *S, float32_t *pSrc,
    float32_t *pDst, uint32_t blockSize);
...
/**
 * @brief  Initialization function for the floating-point transposed direct form
...
void arm_fir_lattice_init_q15(arm_fir_lattice_instance_q15 *S,
                              uint16_t numStages, q15_t *pCoeffs,
                              q15_t *pState);
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_fir_lattice_q15(const arm_fir_lattice_instance_q15 *S, q15_t *pSrc,
                         q15_t *pDst, uint32_t blockSize);
...
void arm_fir_lattice_init_q31(arm_fir_lattice_instance_q31 *S,
                              uint16_t numStages, q31_t *pCoeffs,
                              q31_t *pState);
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_fir_lattice_q31(const arm_fir_lattice_instance_q31 *S, q31_t *pSrc,
                         q31_t *pDst, uint32_t blockSize);
...
void arm_fir_lattice_init_f32(arm_fir_lattice_instance_f32 *S,
                              uint16_t numStages, float32_t *pCoeffs,
                              float32_t *pState);
...
typedef struct {
  ...
  q15_t *pState; /**< points to the state variable array. The array is of length
                    numStages+blockSize. */
  q15_t *pkCoeffs; /**< points to the reflection coefficient array. The array is
                      of length numStages. */
  q15_t *pvCoeffs; /**< points to the ladder coefficient array. The array is of
                      length numStages+1. */
...
typedef struct {
  ...
  q31_t *pState; /**< points to the state variable array. The array is of length
                    numStages+blockSize. */
  q31_t *pkCoeffs; /**< points to the reflection coefficient array. The array is
                      of length numStages. */
  q31_t *pvCoeffs; /**< points to the ladder coefficient array. The array is of
                      length numStages+1. */
...
typedef struct {
  uint16_t numStages;  /**< number of stages in the filter. */
  float32_t *pState;   /**< points to the state variable array. The array is of
                          length numStages+blockSize. */
  float32_t *pkCoeffs; /**< points to the reflection coefficient array. The
                          array is of length numStages. */
  float32_t *pvCoeffs; /**< points to the ladder coefficient array. The array is
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_iir_lattice_f32(const arm_iir_lattice_instance_f32 *S, float32_t *pSrc,
                         float32_t *pDst, uint32_t blockSize);

/**
 * @brief Initialization function for the floating-point IIR lattice filter.
 * @param[in] S          points to an instance of the floating-point IIR lattice
 * structure.
 * @param[in] numStages  number of stages in the filter.
 * @param[in] pkCoeffs   points to the reflection coefficient buffer.  The array
 * is of length numStages.
 * @param[in] pvCoeffs   points to the ladder coefficient buffer.  The array is
 * of length numStages+1.
 * @param[in] pState     points to the state buffer.  The array is of length
 * numStages+blockSize-1.
 * @param[in] blockSize  number of samples to process.
 */
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_iir_lattice_q31(const arm_iir_lattice_instance_q31 *S, q31_t *pSrc,
                         q31_t *pDst, uint32_t blockSize);
 ...
 * @param[in] S          points to an instance of the fixed-point Q15 IIR
 * lattice structure.
 * @param[in] numStages  number of stages in the filter.
 ...
 * @param[in]  S          points to an instance of the floating-point LMS filter
 * structure.
 * @param[in]  pSrc       points to the block of input data.
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_lms_f32(const arm_lms_instance_f32 *S, float32_t *pSrc,
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_lms_q15(const arm_lms_instance_q15 *S, q15_t *pSrc, q15_t *pRef,
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_lms_q31(const arm_lms_instance_q31 *S, q31_t *pSrc, q31_t *pRef,
 ...
 * @param[in]  S          points to an instance of the floating-point normalized
 ...
 * @param[in]  S          points to an instance of the Q31 normalized LMS filter
 * structure.
 * @param[in]  pSrc       points to the block of input data.
 * @param[in]  pRef       points to the block of reference data.
 * @param[out] pOut       points to the block of output data.
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_lms_norm_q31(arm_lms_norm_instance_q31 *S, q31_t *pSrc, q31_t *pRef,
                      q31_t *pOut, q31_t *pErr, uint32_t blockSize);
 ...
 * @param[in]  S          points to an instance of the Q15 normalized LMS filter
 * structure.
 * @param[in]  pSrc       points to the block of input data.
 * @param[in]  pRef       points to the block of reference data.
 * @param[out] pOut       points to the block of output data.
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_lms_norm_q15(arm_lms_norm_instance_q15 *S, q15_t *pSrc, q15_t *pRef,
                      q15_t *pOut, q15_t *pErr, uint32_t blockSize);
 ...
 * @param[in]  pSrcB    points to the second input sequence.
 * @param[in]  srcBLen  length of the second input sequence.
 * @param[out] pDst     points to the block of output data  Length 2 *
 * max(srcALen, srcBLen) - 1.
 */
void arm_correlate_f32(float32_t *pSrcA, uint32_t srcALen, float32_t *pSrcB,
                       uint32_t srcBLen, float32_t *pDst);
 ...
 * @param[out] pDst      points to the block of output data  Length 2 *
 * max(srcALen, srcBLen) - 1.
 * @param[in]  pScratch  points to scratch buffer of size max(srcALen, srcBLen)
 * + 2*min(srcALen, srcBLen) - 2.
 */
void arm_correlate_opt_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
...
void arm_correlate_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
                       uint32_t srcBLen, q15_t *pDst);

/**
 * @brief Correlation of Q15 sequences (fast version) for Cortex-M3 and
 * Cortex-M4.
 * @param[in]  pSrcA    points to the first input sequence.
 * @param[in]  srcALen  length of the first input sequence.
 * @param[in]  pSrcB    points to the second input sequence.
...
void arm_correlate_fast_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
                            uint32_t srcBLen, q15_t *pDst);
 ...
 * + 2*min(srcALen, srcBLen) - 2.
 */
void arm_correlate_fast_opt_q15(q15_t *pSrcA, uint32_t srcALen, q15_t *pSrcB,
                                uint32_t srcBLen, q15_t *pDst, q15_t *pScratch);
 ...
 * max(srcALen, srcBLen) - 1.
 */
void arm_correlate_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
                       uint32_t srcBLen, q31_t *pDst);

/**
 * @brief Correlation of Q31 sequences (fast version) for Cortex-M3 and
 * Cortex-M4
 * @param[in]  pSrcA    points to the first input sequence.
 * @param[in]  srcALen  length of the first input sequence.
 * @param[in]  pSrcB    points to the second input sequence.
 ...
 * max(srcALen, srcBLen) - 1.
 */
void arm_correlate_fast_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
                            uint32_t srcBLen, q31_t *pDst);
 ...
 * max(srcALen, srcBLen) - 1.
 */
void arm_correlate_q7(q7_t *pSrcA, uint32_t srcALen, q7_t *pSrcB,
                      uint32_t srcBLen, q7_t *pDst);
...
typedef struct {
  uint16_t numTaps;    /**< number of coefficients in the filter. */
  uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample in
                          the state buffer. */
  float32_t *pState;   /**< points to the state buffer array. The array is of
                          length maxDelay+blockSize-1. */
  float32_t *pCoeffs;  /**< points to the coefficient array. The array is of
                          length numTaps.*/
  uint16_t maxDelay;   /**< maximum offset specified by the pTapDelay array. */
  int32_t *pTapDelay; /**< points to the array of delay values.  The array is of
                         length numTaps. */
...
typedef struct {
  ...
  uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample in
                          the state buffer. */
  q31_t *pState;  /**< points to the state buffer array. The array is of length
                     maxDelay+blockSize-1. */
  q31_t *pCoeffs; /**< points to the coefficient array. The array is of length
                     numTaps.*/
  uint16_t maxDelay;  /**< maximum offset specified by the pTapDelay array. */
  int32_t *pTapDelay; /**< points to the array of delay values.  The array is of
...
typedef struct {
  ...
  uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample in
                          the state buffer. */
  q15_t *pState;  /**< points to the state buffer array. The array is of length
                     maxDelay+blockSize-1. */
  q15_t *pCoeffs; /**< points to the coefficient array. The array is of length
                     numTaps.*/
  uint16_t maxDelay;  /**< maximum offset specified by the pTapDelay array. */
...
typedef struct {
  ...
  uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample in
                          the state buffer. */
  q7_t *pState;  /**< points to the state buffer array. The array is of length
                    maxDelay+blockSize-1. */
  q7_t *pCoeffs; /**< points to the coefficient array. The array is of length
                    numTaps.*/
  uint16_t maxDelay;  /**< maximum offset specified by the pTapDelay array. */
 ...
 * @param[in]  blockSize   number of input samples to process per call.
 */
void arm_fir_sparse_f32(arm_fir_sparse_instance_f32 *S, float32_t *pSrc,
 ...
 * @param[in]  blockSize   number of input samples to process per call.
 */
void arm_fir_sparse_q31(arm_fir_sparse_instance_q31 *S, q31_t *pSrc,
...
void arm_fir_sparse_init_q31(arm_fir_sparse_instance_q31 *S, uint16_t numTaps,
                             q31_t *pCoeffs, q31_t *pState, int32_t *pTapDelay,
 ...
 * @param[in]  blockSize    number of input samples to process per call.
 */
void arm_fir_sparse_q15(arm_fir_sparse_instance_q15 *S, q15_t *pSrc,
...
void arm_fir_sparse_init_q15(arm_fir_sparse_instance_q15 *S, uint16_t numTaps,
                             q15_t *pCoeffs, q15_t *pState, int32_t *pTapDelay,
                             uint16_t maxDelay, uint32_t blockSize);
 ...
 * @param[in]  blockSize    number of input samples to process per call.
 */
void arm_fir_sparse_q7(arm_fir_sparse_instance_q7 *S, q7_t *pSrc, q7_t *pDst,
 ...
 * control loop mechanism widely used in industrial control systems. A PID
 * controller is the most commonly used type of feedback controller.
 *
 * This set of functions implements (PID) controllers
 * for Q15, Q31, and floating-point data types.  The functions operate on a
 * single sample of data and each call to the function returns a single
 * processed value. <code>S</code> points to an instance of the PID control data
 * structure.  <code>in</code> is the input sample value. The functions return
 * the output value.
 *
 * \par Algorithm:
 * <pre>
 ...
 * control inputs. The proportional value determines the reaction to the current
 * error, the integral value determines the reaction based on the sum of recent
 * errors, and the derivative value determines the reaction based on the rate at
 ...
 * defined for each PID Controller. There are separate instance structure
 ...
 * Care must be taken when using the fixed-point versions of the PID Controller
 * functions. In particular, the overflow and saturation behavior of the
 ...
 */
CMSIS_INLINE __STATIC_INLINE float32_t arm_pid_f32(arm_pid_instance_f32 *S,
                                                   float32_t in) {
  float32_t out;
 ...
 * by 2 bits as there are four additions. After all multiply-accumulates are
 * performed, the 2.62 accumulator is truncated to 1.32 format and then
 * saturated to 1.31 format.
 */
CMSIS_INLINE __STATIC_INLINE q31_t arm_pid_q31(arm_pid_instance_q31 *S,
                                               q31_t in) {
  q63_t acc;
  q31_t out;
 ...
 */
CMSIS_INLINE __STATIC_INLINE q15_t arm_pid_q15(arm_pid_instance_q15 *S,
                                               q15_t in) {
  q63_t acc;
  q15_t out;
 ...
 * three-phase currents <code>Ia, Ib and Ic</code> to calculate currents in the
 * two-phase orthogonal stator axis <code>Ialpha</code> and <code>Ibeta</code>.
 * When <code>Ialpha</code> is superposed with <code>Ia</code> as shown in the
 * figure below \image html clarke.gif Stator current space vector and its
 * components in (a,b). and <code>Ia + Ib + Ic = 0</code>, in this condition
 * <code>Ialpha</code> and <code>Ibeta</code> can be calculated using only
 * <code>Ia</code> and <code>Ib</code>.
 *
 ...
 * clarkeFormula.gif where <code>Ia</code> and <code>Ib</code> are the
 * instantaneous stator phases and <code>pIalpha</code> and <code>pIbeta</code>
 * are the two coordinates of time invariant vector. \par Fixed-Point Behavior
 ...
 */
CMSIS_INLINE __STATIC_INLINE void arm_clarke_f32(float32_t Ia, float32_t Ib,
                                                 float32_t *pIalpha,
                                                 float32_t *pIbeta) {
  /* Calculate pIalpha using the equation, pIalpha = Ia */
  *pIalpha = Ia;

  /* Calculate pIbeta using the equation, pIbeta = (1/sqrt(3)) * Ia +
   * (2/sqrt(3)) * Ib */
 ...
 * intermediate multiplication in 2.62 format. There is saturation on the
 ...
 * Inverse Clarke transform converts the two-coordinate time invariant vector
 * into instantaneous stator phases.
 *
 * The function operates on a single sample of data and each call to the
 * function returns the processed output. The library provides separate
 * functions for Q31 and floating-point data types. \par Algorithm \image html
 * clarkeInvFormula.gif where <code>pIa</code> and <code>pIb</code> are the
 * instantaneous stator phases and <code>Ialpha</code> and <code>Ibeta</code>
 * are the two coordinates of time invariant vector. \par Fixed-Point Behavior
 * Care must be taken when using the Q31 version of the Clarke transform.
 * In particular, the overflow and saturation behavior of the accumulator used
 * must be considered. Refer to the function specific documentation below for
...
CMSIS_INLINE __STATIC_INLINE void arm_inv_clarke_f32(float32_t Ialpha,
  ...
  *pIa = Ialpha;
  ...
  /* Calculating pIb from Ialpha and Ibeta by equation pIb = -(1/2) * Ialpha +
   * (sqrt(3)/2) * Ibeta */
 ...
 */
CMSIS_INLINE __STATIC_INLINE void arm_inv_clarke_q31(q31_t Ialpha, q31_t Ibeta,
                                                     q31_t *pIa, q31_t *pIb) {
  q31_t product1,
      product2; /* Temporary variables used to store intermediate results */
 ...
 * transformation of the <code>Ialpha</code> and the <code>Ibeta</code> currents
 * from the stationary to the moving reference frame and control the spatial
 * relationship between the stator vector current and rotor flux vector. If we
 * consider the d axis aligned with the rotor flux, the diagram below shows the
 ...
 * function returns the processed output. The library provides separate
 * functions for Q31 and floating-point data types. \par Algorithm \image html
 * parkFormula.gif where <code>Ialpha</code> and <code>Ibeta</code> are the
 * stator vector components, <code>pId</code> and <code>pIq</code> are rotor
 * vector components and <code>cosVal</code> and <code>sinVal</code> are the
...
CMSIS_INLINE __STATIC_INLINE void arm_park_f32(float32_t Ialpha,
                                               float32_t Ibeta, float32_t *pId,
                                               float32_t *pIq, float32_t sinVal,
                                               float32_t cosVal) {
 ...
 */
CMSIS_INLINE __STATIC_INLINE void arm_park_q31(q31_t Ialpha, q31_t Ibeta,
                                               q31_t *pId, q31_t *pIq,
 ...
 * function returns the processed output. The library provides separate
 * functions for Q31 and floating-point data types. \par Algorithm \image html
 * parkInvFormula.gif where <code>pIalpha</code> and <code>pIbeta</code> are the
 * stator vector components, <code>Id</code> and <code>Iq</code> are rotor
 * vector components and <code>cosVal</code> and <code>sinVal</code> are the
 * cosine and sine values of theta (rotor flux position).
 * \par Fixed-Point Behavior
 * Care must be taken when using the Q31 version of the Park transform.
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_inv_park_f32(float32_t Id, float32_t Iq, float32_t *pIalpha,
 ...
 * addition, hence there is no risk of overflow.
 */
CMSIS_INLINE __STATIC_INLINE void arm_inv_park_q31(q31_t Id, q31_t Iq,
                                                   q31_t *pIalpha,
                                                   q31_t *pIbeta, q31_t sinVal,
                                                   q31_t cosVal) {
  q31_t product1,
      product2; /* Temporary variables used to store intermediate results */
  q31_t product3,
 ...
 * Linear interpolation works by effectively drawing a straight line between two
 * neighboring samples and returning the appropriate point along that line
 *
 * \par
 * \image html LinearInterp.gif "Linear interpolation"
 *
 * \par
 * A  Linear Interpolate function calculates an output value(y), for the
 * input(x) using linear interpolation of the input values x0, x1( nearest input
 * values) and the output values y0 and y1(nearest output values)
 *
 * \par Algorithm:
 * <pre>
 ...
 * for Q7, Q15, Q31, and floating-point data types.  The functions operate on a
 * single sample of data and each call to the function returns a single
 * processed value. <code>S</code> points to an instance of the Linear
 * Interpolate function data structure. <code>x</code> is the input sample
 * value. The functions returns the output value.
 *
 * \par
 * if x is outside of the table boundary, Linear interpolation returns first
 * value of the table if x is below input range and returns last value of table
 * if x is above range.
 */
...
/**
 * @brief  Process function for the floating-point Linear Interpolation
 * Function.
 * @param[in,out] S  is an instance of the floating-point Linear Interpolation
 * structure
 ...
 *
 * \par
 * Input sample <code>x</code> is in 12.20 format which contains 12 bits for
 * table index and 20 bits for fractional part. This function can support
 * maximum of table size 2^12.
 *
 */
CMSIS_INLINE __STATIC_INLINE q31_t arm_linear_interp_q31(q31_t *pYData, q31_t x,
                                                         uint32_t nValues) {
  q31_t y;       /* output */
 ...
 *
 */
CMSIS_INLINE __STATIC_INLINE q15_t arm_linear_interp_q15(q15_t *pYData, q31_t x,
                                                         uint32_t nValues) {
  q63_t y;       /* output */
 ...
 * maximum of table size 2^12.
 */
CMSIS_INLINE __STATIC_INLINE q7_t arm_linear_interp_q7(q7_t *pYData, q31_t x,
                                                       uint32_t nValues) {
  q31_t y;        /* output */
 ...
 * <code>f'(x0)</code> is the derivative of <code>f()</code> evaluated at
 * <code>x0</code>. For the square root function, the algorithm reduces to:
 * <pre>
 ...
 * value or ARM_MATH_ARGUMENT_ERROR if <code>in</code> is negative value and
 * returns zero output for negative values.
 */
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularWrite_f32(int32_t *circBuffer, int32_t L, uint16_t *writeOffset,
                      int32_t bufferInc, const int32_t *src, int32_t srcInc,
                      uint32_t blockSize) {
  uint32_t i = 0U;
  int32_t wOffset;
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularRead_f32(int32_t *circBuffer, int32_t L, int32_t *readOffset,
                     int32_t bufferInc, int32_t *dst, int32_t *dst_base,
                     int32_t dst_length, int32_t dstInc, uint32_t blockSize) {
  uint32_t i = 0U;
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularWrite_q15(q15_t *circBuffer, int32_t L, uint16_t *writeOffset,
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularRead_q15(q15_t *circBuffer, int32_t L, int32_t *readOffset,
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularWrite_q7(q7_t *circBuffer, int32_t L, uint16_t *writeOffset,
 ...
 */
CMSIS_INLINE __STATIC_INLINE void
arm_circularRead_q7(q7_t *circBuffer, int32_t L, int32_t *readOffset,
 ...
 * @param[out] imagResult  imaginary part of the result returned here
 */
void arm_cmplx_dot_prod_f32(float32_t *pSrcA, float32_t *pSrcB,
 ...
 */
void arm_cmplx_mult_real_q15(q15_t *pSrcCmplx, q15_t *pSrcReal,
 ...
 * @param[in]  numSamples  number of samples in each vector
 */
void arm_cmplx_mult_real_q31(q31_t *pSrcCmplx, q31_t *pSrcReal,
 ...
 * @param[in]  numSamples  number of samples in each vector
 */
void arm_cmplx_mult_real_f32(float32_t *pSrcCmplx, float32_t *pSrcReal,
 ...
 * @param[in]  index      is the array index of the minimum value in the input
 * buffer.
 */
 ...
 * @param[in]  pIndex     is the array index of the minimum value in the input
 * buffer.
 */
void arm_min_q15(q15_t *pSrc, uint32_t blockSize, q15_t *pResult,
                 uint32_t *pIndex);
 ...
 * @param[in]  blockSize  is the number of samples to process
 * @param[out] pResult    is output pointer
 * @param[out] pIndex     is the array index of the minimum value in the input
 * buffer.
 */
void arm_min_q31(q31_t *pSrc, uint32_t blockSize, q31_t *pResult,
                 uint32_t *pIndex);
 ...
 */
void arm_max_q7(q7_t *pSrc, uint32_t blockSize, q7_t *pResult,
                uint32_t *pIndex);
 ...
 * @param[out] pIndex     index of maximum value returned here
 */
void arm_max_q31(q31_t *pSrc, uint32_t blockSize, q31_t *pResult,
                 uint32_t *pIndex);
 ...
 *
 * Bilinear interpolation is an extension of linear interpolation applied to a
 * two dimensional grid. The underlying function <code>f(x, y)</code> is sampled
 * on a regular grid and the interpolation process determines values between the
 * grid points. Bilinear interpolation is equivalent to two step linear
 * interpolation, first in the x-dimension and then in the y-dimension. Bilinear
 * interpolation is often used in image processing to rescale images. The CMSIS
 * DSP library provides bilinear interpolation functions for Q7, Q15, Q31, and
 * floating-point data types.
 *
 * <b>Algorithm</b>
 * \par
 * The instance structure used by the bilinear interpolation functions describes
 * a two dimensional data table. For floating-point, the instance structure is
 * defined as: <pre> typedef struct
 *   {
 *     uint16_t numRows;
 *     uint16_t numCols;
 *     float32_t *pData;
 ...
 * and <code>pData</code> points to an array of size
 * <code>numRows*numCols</code> values. The data table <code>pTable</code> is
 * organized in row order and the supplied data values fall on integer indexes.
 * That is, table element (x,y) is located at <code>pTable[x + y*numCols]</code>
 * where x and y are integers.
 *
 * \par
 * Let <code>(x, y)</code> specify the desired interpolation point.  Then
 * define: <pre> XF = floor(x) YF = floor(y)
 * </pre>
 * \par
 ...
 *
 * \par
 * if (x,y) are outside of the table boundary, Bilinear interpolation returns
 * zero output.
 */
...
arm_bilinear_interp_q31(arm_bilinear_interp_instance_q31 *S, q31_t X, q31_t Y) {
...
arm_bilinear_interp_q15(arm_bilinear_interp_instance_q15 *S, q31_t X, q31_t Y) {
  q63_t acc = 0;               /* output */
  q31_t out;                   /* Temporary output */
...
arm_bilinear_interp_q7(arm_bilinear_interp_instance_q7 *S, q31_t X, q31_t Y) {
  q63_t acc = 0;               /* output */
  q31_t out;                   /* Temporary output */
  q31_t xfract, yfract;        /* X, Y fractional parts */
...
/* Exit low optimization region - place directly after end of function
 * definition */
#if defined(ARM_MATH_CM4) || defined(ARM_MATH_CM7)
