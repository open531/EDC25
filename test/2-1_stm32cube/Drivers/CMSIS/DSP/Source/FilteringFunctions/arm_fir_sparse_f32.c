 ...
 * The functions operate on blocks  of input and output data and each call to
 *the function processes <code>blockSize</code> samples through the filter.
 ...
 *<code>pTapDelay</code> which specifies the locations of the non-zero
 ...
 * </pre>
 * \par
 * \image html FIRSparse.gif "Sparse FIR filter.  b[n] represents the filter
 *coefficients" \par <code>pCoeffs</code> points to a coefficient array of size
 ...
 * <code>maxDelay</code> is the largest offset value that is ever used in the
 *<code>pTapDelay</code> array. Some of the processing functions also require
 ...
 *each filter. Coefficient and offset arrays may be shared among several
 ...
 *subfields of the instance structure: numTaps, pCoeffs, pTapDelay, maxDelay,
 ...
 *data type filter instance structures <pre> arm_fir_sparse_instance_f32 S =
 *{numTaps, 0, pState, pCoeffs, maxDelay, pTapDelay};
 *arm_fir_sparse_instance_q31 S = {numTaps, 0, pState, pCoeffs, maxDelay,
 *pTapDelay}; arm_fir_sparse_instance_q15 S = {numTaps, 0, pState, pCoeffs,
 *maxDelay, pTapDelay}; arm_fir_sparse_instance_q7 S =  {numTaps, 0, pState,
 *pCoeffs, maxDelay, pTapDelay};
 * </pre>
 * \par
 *
 * \par Fixed-Point Behavior
 * Care must be taken when using the fixed-point versions of the sparse FIR
