 ...
 * Scale down one of the inputs by 1/min(srcALen, srcBLen)to avoid overflows
 * since a maximum of min(srcALen, srcBLen) number of additions is carried
 * internally. The 2.62 accumulator is right shifted by 31 bits and saturated
 * to 1.31 format to yield the final result.
 *
 * \par
 * See <code>arm_correlate_fast_q31()</code> for a faster but less precise
 * implementation of this function for Cortex-M3 and Cortex-M4.
 */
...
void arm_correlate_q31(q31_t *pSrcA, uint32_t srcALen, q31_t *pSrcB,
  ...
  if (srcBLen >= 4U) {
    ...
    /* If the blockSize2 is not a multiple of 3, compute any remaining output
     *samples here.
     ** No loop unrolling is used. */
