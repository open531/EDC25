 ...
 * using a transposed direct form II structure. The filters are implemented as a
 ...
 * equation: <pre> y[n] = b0 * x[n] + d1 d1 = b1 * x[n] + a1 * y[n] + d2 d2 = b2
 * * x[n] + a2 * y[n]
 * </pre>
 ...
 * Coefficients <code>b0, b1, and b2 </code> multiply the input signal
 * <code>x[n]</code> and are referred to as the feedforward coefficients.
 * Coefficients <code>a1</code> and <code>a2</code> multiply the output signal
 * <code>y[n]</code> and are referred to as the feedback coefficients. Pay
 * careful attention to the sign of the feedback coefficients. Some design tools
 * flip the sign of the feedback coefficients: <pre> y[n] = b0 * x[n] + d1; d1 =
 * b1 * x[n] - a1 * y[n] + d2; d2 = b2 * x[n] - a2 * y[n];
 * </pre>
 ...
 *
 * \par
 * The CMSIS library contains Biquad filters in both Direct Form I and
 * transposed Direct Form II. The advantage of the Direct Form I structure is
 * that it is numerically more robust for fixed-point data types. That is why
 * the Direct Form I structure supports Q15 and Q31 data types. The transposed
 * Direct Form II structure, on the other hand, requires a wide dynamic range
 * for the state variables <code>d1</code> and <code>d2</code>. Because of this,
 * the CMSIS library only has a floating-point version of the Direct Form II
 * Biquad. The advantage of the Direct Form II Biquad is that it requires half
 * the number of state variables, 2 rather than 4, per Biquad stage.
 *
 ...
 * initialization. For example, to statically initialize the instance structure
 * use <pre> arm_biquad_cascade_df2T_instance_f32 S1 = {numStages, pState,
 * pCoeffs};
 * </pre>
...
void arm_biquad_cascade_stereo_df2T_f32(
  ...
  float32_t Xn2a, Xn3a, Xn4a, Xn5a, Xn6a, Xn7a,
      Xn8a; /*  Input State variables     */
  float32_t Xn2b, Xn3b, Xn4b, Xn5b, Xn6b, Xn7b,
      Xn8b; /*  Input State variables     */
  float32_t acc2a, acc3a, acc4a, acc5a, acc6a, acc7a,
      acc8a; /*  Simulates the accumulator */
  float32_t acc2b, acc3b, acc4b, acc5b, acc6b, acc7b,
      acc8b; /*  Simulates the accumulator */
