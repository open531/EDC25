 ...
 * This leads to loss of precision on the intermediate multiplications and
 * provides only a single guard bit. If the accumulator result overflows, it
 * wraps around rather than saturate. In order to avoid overflows the input
 * signal or coefficients must be scaled down by log2(numTaps) bits.
 */
