 ...
 *
 * \par
 * After each sample of the error signal is computed, the filter coefficients
 *<code>b[k]</code> are updated on a sample-by-sample basis: <pre> b[k] = b[k] +
 *e[n] * mu * x[n-k],  for k=0, 1, ..., numTaps-1
 * </pre>
 ...
 *subfields of the instance structure: numTaps, pCoeffs, mu, postShift (not for
 *f32), pState. Also set all of the values in pState to zero.
 *
 * \par
 ...
 *data type filter instance structures <pre> arm_lms_instance_f32 S = {numTaps,
 *pState, pCoeffs, mu}; arm_lms_instance_q31 S = {numTaps, pState, pCoeffs, mu,
 *postShift}; arm_lms_instance_q15 S = {numTaps, pState, pCoeffs, mu,
 *postShift};
 * </pre>
