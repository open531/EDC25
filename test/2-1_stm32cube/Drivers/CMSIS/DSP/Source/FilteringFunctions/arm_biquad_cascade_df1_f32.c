 ...
 * and <code>y[n-2]</code>. The state variables are arranged in the
 * <code>pState</code> array as: <pre> {x[n-1], x[n-2], y[n-1], y[n-2]}
 * </pre>
 *
 * \par
 ...
 * initialization. The code below statically initializes each of the 3 different
 * data type filter instance structures <pre> arm_biquad_casd_df1_inst_f32 S1 =
 * {numStages, pState, pCoeffs}; arm_biquad_casd_df1_inst_q15 S2 = {numStages,
 * pState, pCoeffs, postShift}; arm_biquad_casd_df1_inst_q31 S3 = {numStages,
 ...
 * Care must be taken when using the Q15 and Q31 versions of the Biquad Cascade
 * filter functions. Following issues must be considered:
 ...
 * The fixed-point functions have an additional scaling parameter
 * <code>postShift</code> which allow the filter coefficients to exceed the
 ...
 * For Q15 and Q31 versions, it is described separately as part of the function
...
void arm_biquad_cascade_df1_f32(const arm_biquad_casd_df1_inst_f32 *S,
  ...
  do {
     ...
     * * y[n-2] acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 * y[n-1]   +
     * a2 * y[n-2] acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 * y[n-1] +
     * a2 * y[n-2]
     */
