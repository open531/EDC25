 ...
 * time reversed order: <pre> {b[numTaps-1], b[numTaps-2], b[numTaps-2], ...,
 * b[1], b[0]}
 * </pre>
 * The length of the filter <code>numTaps</code> must be a multiple of the
 * interpolation factor <code>L</code>. \par <code>pState</code> points to the
 * array of state variables. <code>pState</code> is of length
 * <code>(numTaps/L)+blockSize-1</code> words where <code>blockSize</code> is
 * the number of input samples processed by each call to
...
arm_status arm_fir_interpolate_init_f32(arm_fir_interpolate_instance_f32 *S,
  ...
  if ((numTaps % L) != 0U) {
    ...
    /* Clear state buffer and size of state array is always phaseLength +
     * blockSize - 1 */
    memset(pState, 0,
