 ...
 * that one of the inputs is flipped in convolution. Correlation is commonly
 * used to measure the similarity between two signals. It has applications in
 * pattern recognition, cryptanalysis, and searching. The CMSIS library provides
 ...
 * <code>srcALen</code> and <code>srcBLen</code> samples respectively. The
 ...
 * \par
 * The <code>pSrcA</code> points to the first input vector of length
 * <code>srcALen</code> and <code>pSrcB</code> points to the second input vector
 * of length <code>srcBLen</code>. The result <code>c[n]</code> is of length
 * <code>2 * max(srcALen, srcBLen) - 1</code> and is defined over the interval
 * <code>n=0, 1, 2, ..., (2 * max(srcALen, srcBLen) - 2)</code>. The output
 * result is written to <code>pDst</code> and the calling function must allocate
 * <code>2 * max(srcALen, srcBLen) - 1</code> words for the result.
 *
 * <b>Note</b>
 * \par
 ...
 * less compared to Q31 and Q15 of correlate and the design requires the input
 ...
 * and consumes more memory(Scratch memory) compared to Q15 and Q7 versions of
 * correlate
 */
...
void arm_correlate_f32(float32_t *pSrcA, uint32_t srcALen, float32_t *pSrcB,
   ...
   * divided into three parts according to the number of multiplications that
   * has to be taken place between inputA samples and inputB samples. In the
   * first part of the algorithm, the multiplications increase by one for every
   * iteration. In the second part of the algorithm, srcBLen number of
   * multiplications are done. In the third part of the algorithm, the
   ...
   * than or equal to 4, to loop unroll the srcBLen loop */
  if (srcBLen >= 4U) {
