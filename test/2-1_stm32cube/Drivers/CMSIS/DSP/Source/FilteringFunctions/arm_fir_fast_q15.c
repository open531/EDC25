 ...
 * results but provides only a single guard bit. Thus, if the accumulator result
 * overflows it wraps around and distorts the result. In order to avoid
 * overflows completely the input signal must be scaled down by log2(numTaps)
 * bits. The 2.30 accumulator is then truncated to 2.15 format and saturated to
 * yield the 1.15 result.
 *
 * \par
 * Refer to the function <code>arm_fir_q15()</code> for a slower implementation
 * of this function which uses 64-bit accumulation to avoid wrap around
 * distortion.  Both the slow and the fast versions use the same instance
 * structure. Use the function <code>arm_fir_init_q15()</code> to initialize the
 * filter structure.
 */
...
void arm_fir_fast_q15(const arm_fir_instance_q15 *S, q15_t *pSrc, q15_t *pDst,
  ...
  q31_t x0, x1, x2,
      c0; /* Temporary variables to hold SIMD state and coefficient values */
