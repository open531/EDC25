 ...
 * high frequency copies of the spectrum. The user of the function is
 ...
 * algorithms. With this method the computation is reduced by a factor of
 * <code>1/L</code> when compared to using a standard FIR filter. \par
 ...
 * </pre>
 * \par
 * <code>pState</code> points to a state array of size <code>blockSize +
 * phaseLength - 1</code>. Samples in the state buffer are stored in the order:
 * \par
 * <pre>
 *    {x[n-phaseLength+1], x[n-phaseLength], x[n-phaseLength-1],
 * x[n-phaseLength-2]....x[0], x[1], ..., x[blockSize-1]}
 * </pre>
 ...
 * interpolation factor. To do this manually without calling the init function,
 * assign the follow subfields of the instance structure: L (interpolation
 * factor), pCoeffs, phaseLength (numTaps / L), pState. Also set all of the
 * values in pState to zero.
 *
 * \par
 ...
 * data type filter instance structures <pre> arm_fir_interpolate_instance_f32 S
 * = {L, phaseLength, pCoeffs, pState}; arm_fir_interpolate_instance_q31 S = {L,
 * phaseLength, pCoeffs, pState}; arm_fir_interpolate_instance_q15 S = {L,
 * phaseLength, pCoeffs, pState};
 * </pre>
 ...
 * coefficient buffer; <code>pState</code> is the address of the state buffer.
 * Be sure to set the values in the state buffer to zeros when doing static
 * initialization.
 *
...
void arm_fir_interpolate_f32(const arm_fir_interpolate_instance_f32 *S,
  ...
  float32_t *ptr1,
      *ptr2;      /* Temporary pointers for state and coefficient buffers */
