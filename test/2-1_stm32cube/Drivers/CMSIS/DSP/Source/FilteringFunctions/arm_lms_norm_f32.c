 ...
 *accept the input and reference input signals and generate the filter output
 *and error signal. \image html LMS.gif "Internal structure of the NLMS adaptive
 *filter"
 *
 ...
 *
 * \par
 * The error signal equals the difference between the reference signal
 *<code>d[n]</code> and the filter output: <pre> e[n] = d[n] - y[n].
 * </pre>
 *
 * \par
 * After each sample of the error signal is computed the instanteous energy of
 *the filter state variables is calculated: <pre> E = x[n]^2 + x[n-1]^2 + ... +
 *x[n-numTaps+1]^2.
 * </pre>
 * The filter coefficients <code>b[k]</code> are then updated on a
 *sample-by-sample basis: <pre> b[k] = b[k] + e[n] * (mu/E) * x[n-k],  for k=0,
 *1, ..., numTaps-1
 * </pre>
 * where <code>mu</code> is the step size and controls the rate of coefficient
 *convergence. \par In the APIs, <code>pCoeffs</code> points to a coefficient
 *array of size <code>numTaps</code>. Coefficients are stored in time reversed
 *order. \par <pre> {b[numTaps-1], b[numTaps-2], b[N-2], ..., b[1], b[0]}
 * </pre>
 * \par
 * <code>pState</code> points to a state array of size <code>numTaps + blockSize
 *- 1</code>. Samples in the state buffer are stored in the order: \par <pre>
 ...
 *coefficient array by <code>blockSize-1</code> samples. The increased state
 *buffer length allows circular addressing, which is traditionally used in FIR
 *filters, to be avoided and yields a significant speed improvement. The state
 *variables are updated after each block of data is processed. \par Instance
 *Structure The coefficients and state variables for a filter are stored
 ...
 *subfields of the instance structure: numTaps, pCoeffs, mu, energy, x0, pState.
 *Also set all of the values in pState to zero. For Q7, Q15, and Q31 the
 *following fields must also be initialized; recipTable, postShift
 *
 * \par
 * Instance structure cannot be placed into a const data section and it is
 *recommended to use the initialization function. \par Fixed-Point Behavior:
 * Care must be taken when using the Q15 and Q31 versions of the normalised LMS
 ...
 * The value of <code>postShift</code> is set by the user based on the expected
 *gain through the system being modeled.
 *
 ...
 * @param[in] *S points to an instance of the floating-point normalized LMS
 * filter structure.
...
void arm_lms_norm_f32(arm_lms_norm_instance_f32 *S, float32_t *pSrc,
  ...
  float32_t w, x0,
      in; /* weight factor, temporary variable to hold input sample and state */
