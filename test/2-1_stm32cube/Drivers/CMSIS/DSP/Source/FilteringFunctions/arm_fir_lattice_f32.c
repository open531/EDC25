 ...
 *coefficients are untouched. \par Instance Structure The coefficients and state
 *variables for a filter are stored together in an instance data structure. A
 *separate instance structure must be defined for each filter. Coefficient
 *arrays may be shared among several instances while state variable arrays
 *cannot be shared. There are separate instance structure declarations for each
 ...
 *is the address of the coefficient buffer. \par Fixed-Point Behavior Care must
 *be taken when using the fixed-point versions of the FIR Lattice filter
