 ...
 * accumulator in 18.14 format. This approach provides 17 guard bits and there
 * is no risk of overflow as long as <code>max(srcALen, srcBLen)<131072</code>.
 * The 18.14 result is then truncated to 18.7 format by discarding the low 7
 * bits and saturated to 1.7 format.
 *
 * \par
 * Refer the function <code>arm_correlate_opt_q7()</code> for a faster
