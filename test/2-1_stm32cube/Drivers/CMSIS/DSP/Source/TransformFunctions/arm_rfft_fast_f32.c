...
void merge_rfft_f32(arm_rfft_fast_instance_f32 *S, float32_t *p,
                    float32_t *pOut) {
  uint32_t k;                          /* Loop Counter                     */
  float32_t twR, twI;                  /* RFFT Twiddle coefficients        */
  float32_t *pCoeff = S->pTwiddleRFFT; /* Points to RFFT Twiddle factors   */
 ...
 * \par
 * The Fast RFFT algorith relays on the mixed radix CFFT that save processor
 *usage. \par The real length N forward FFT of a sequence is computed using the
 *steps shown below. \par \image html RFFT.gif "Real Fast Fourier Transform"
 * \par
 * The real sequence is initially treated as if it were complex to perform a
 *CFFT. Later, a processing stage reshapes the data to obtain half of the
 *frequency spectrum in complex format. Except the first complex number that
 *contains the two real numbers X[0] and X[N/2] all the data is complex. In
 *other words, the first complex sample contains two real values packed. \par
 * The input for the inverse RFFT should keep the same format as the output of
 *the forward RFFT. A first processing stage pre-process the data to later
 *perform an inverse CFFT. \par \image html RIFFT.gif "Real Inverse Fast Fourier
 *Transform" \par The algorithms for floating-point, Q15, and Q31 data are
 *slightly different and we describe each algorithm in turn. \par Floating-point
 ...
 * <pre>
 *arm_rfft_instance_q31 S = {fftLenReal, fftLenBy2, ifftFlagR, bitReverseFlagR,
 *twidCoefRModifier, pTwiddleAReal, pTwiddleBReal, pCfft}; arm_rfft_instance_q15
 *S = {fftLenReal, fftLenBy2, ifftFlagR, bitReverseFlagR, twidCoefRModifier,
 ...
 * must also be initialized.  Refer to arm_cfft_radix4_f32() for details
 *regarding static initialization of the complex FFT instance structure.
 */
 ...
 * @param[in]  *S              points to an arm_rfft_fast_instance_f32
 * structure.
