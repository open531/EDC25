 ...
 * Representation of signals by minimum number of values is important for
 *storage and transmission. The possibility of large discontinuity between the
 *beginning and end of a period of a signal in DFT can be avoided by extending
 *the signal so that it is even-symmetric. Discrete Cosine Transform (DCT) is
 *constructed such that its energy is heavily concentrated in the lower part of
 *the spectrum and is very widely used in signal and image coding applications.
 * The family of DCTs (DCT type- 1,2,3,4) is the outcome of different
 *combinations of homogeneous boundary conditions. DCT has an excellent
 *energy-packing capability, hence has many applications and in data compression
 *in particular.
 *
 ...
 * - Multiplication of weights and Real FFT output and getting real part from
 *the product.
 *
 ...
 *
 * \par Algorithm:
 * The N-point type-IV DCT is defined as a real, linear transformation by the
 *formula: \image html DCT4Equation.gif where <code>k = 0,1,2,.....N-1</code>
 *\par
 ...
 *multiplying with an overall scale factor of sqrt(2/N). The symmetry of the
 *transform matrix indicates that the fast algorithms for the forward and
 *inverse transform computation are identical. Note that the implementation of
 *Inverse DCT4 and DCT4 is same, hence same process function can be used for
 *both.
 *
 * \par Lengths supported by the transform:
 *  As DCT4 internally uses Real FFT, it supports all the lengths 128, 512, 2048
 *and 8192. The library provides separate functions for Q15, Q31, and
 *floating-point data types. \par Instance Structure The instances for Real FFT
 *and FFT, cosine values table and twiddle factor table are stored in an
 *instance data structure. A separate instance structure must be defined for
 *each transform. There are separate instance structure declarations for each of
 ...
 * - Initializes Real FFT as its process function is used internally in DCT4, by
 *calling arm_rfft_init_f32(). \par Use of the initialization function is
 *optional. However, if the initialization function is used, then the instance
 *structure cannot be placed into a const data section. To place an instance
 *structure into a const data section, the instance structure must be manually
 *initialized. Manually initialize the instance structure as follows: <pre>
 *arm_dct4_instance_f32 S = {N, Nby2, normalize, pTwiddle, pCosFactor, pRfft,
 *pCfft}; arm_dct4_instance_q31 S = {N, Nby2, normalize, pTwiddle, pCosFactor,
 *pRfft, pCfft}; arm_dct4_instance_q15 S = {N, Nby2, normalize, pTwiddle,
 *pCosFactor, pRfft, pCfft};
 * </pre>
 * where \c N is the length of the DCT4; \c Nby2 is half of the length of the
 *DCT4; \c normalize is normalizing factor used and is equal to
 ...
 * Care must be taken when using the fixed-point versions of the DCT4 transform
 ...
 * @param[in,out]   *pInlineBuffer points to the in-place input and output
 * buffer.
 * @return none.
 */
