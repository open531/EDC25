 ...
 * <code>c</code> is <code>pi/(2*N)</code> \par Converted the output to q15
 * format by multiplying with 2^31 and saturated if required. \par In the tables
 * below the real and imaginary values are placed alternatively, hence the array
 ...
 * transform length. \par Normalizing factor: The normalizing factor is
 * <code>sqrt(2/N)</code>, which depends on the size of transform
 * <code>N</code>. Normalizing factors in 1.15 format are mentioned in the table
 * below for different DCT sizes: \image html dct4NormalizingQ15Table.gif
 */
