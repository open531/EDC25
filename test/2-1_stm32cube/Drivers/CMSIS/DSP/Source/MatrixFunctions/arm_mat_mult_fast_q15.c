 ...
 *
 * \par
 * The difference between the function arm_mat_mult_q15() and this fast variant
 * is that the fast variant use a 32-bit rather than a 64-bit accumulator. The
 * result of each 1.15 x 1.15 multiplication is truncated to 2.30 format. These
 * intermediate results are accumulated in a 32-bit register in 2.30 format.
 * Finally, the accumulator is saturated and converted to a 1.15 result.
 *
 * \par
 * The fast version has the same overflow behavior as the standard version but
 * provides less precision since it discards the low 16 bits of each
 * multiplication result. In order to avoid overflows completely the input
 * signals must be scaled down. Scale down one of the input matrices by
 * log2(numColsA) bits to avoid overflows, as a total of numColsA additions are
 * computed internally for each output element.
 *
 * \par
 * See <code>arm_mat_mult_q15()</code> for a slower implementation of this
 * function which uses 64-bit accumulation to provide higher precision.
 */
...
#endif
  {
    ...
    while (row > 0U) {
      ...
      while (col > 0U) {
        ...
        /* Initiate the pointer pInA to point to the starting address of the
