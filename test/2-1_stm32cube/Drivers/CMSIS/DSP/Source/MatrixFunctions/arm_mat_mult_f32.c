 ...
 * When matrix size checking is enabled, the functions check: (1) that the inner
 dimensions of
 * <code>pSrcA</code> and <code>pSrcB</code> are equal; and (2) that the size of
 the output
 * matrix equals the outer dimensions of <code>pSrcA</code> and
 <code>pSrcB</code>.
 */
...
arm_status arm_mat_mult_f32(const arm_matrix_instance_f32 *pSrcA,
  ...
  {
    /* The following loop performs the dot-product of each row in pInA with each
     * column in pInB */
    /* row loop */
    do {
      /* Output pointer is set to starting address of the row being processed */
