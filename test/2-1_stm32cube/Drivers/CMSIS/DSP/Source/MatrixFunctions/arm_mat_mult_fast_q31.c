 ...
 *
 * \par
 * The difference between the function arm_mat_mult_q31() and this fast variant
 * is that the fast variant use a 32-bit rather than a 64-bit accumulator. The
 * result of each 1.31 x 1.31 multiplication is truncated to 2.30 format. These
 ...
 * provides less precision since it discards the low 32 bits of each
 ...
 *
 * \par
 * See <code>arm_mat_mult_q31()</code> for a slower implementation of this
