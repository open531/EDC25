 ...
 * <code>ARM_MATH_SIZE_MISMATCH</code> if the input matrix is not square or if
 * the size of the output matrix does not match the size of the input matrix. If
 * the input matrix is found to be singular (non-invertible), then the function
 * returns <code>ARM_MATH_SINGULAR</code>.  Otherwise, the function returns
