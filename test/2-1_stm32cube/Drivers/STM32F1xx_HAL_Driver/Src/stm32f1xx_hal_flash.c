/**
  ...
  *
  ...
  FLASH memory of all STM32F1xx devices.
  ...
  [..] In addition to these function, this driver includes a set of macros
 ...
 * @note   The function HAL_FLASH_Unlock() should be called before to unlock the
 * FLASH interface The function HAL_FLASH_Lock() should be called after to lock
 * the FLASH interface
 *
 * @note   If an erase and a program operations are requested simultaneously,
 *         the erase operation is performed before the program one.
 *
 * @note   FLASH should be previously erased before new programmation (only
 * exception to this is when 0x0000 is programmed)
 *
...
/**
 * @brief  Program halfword, word or double word at a specified address  with
 * interrupt enabled.
 * @note   The function HAL_FLASH_Unlock() should be called before to unlock the
 * FLASH interface The function HAL_FLASH_Lock() should be called after to lock
 * the FLASH interface
...
void HAL_FLASH_IRQHandler(void) {
...
#if defined(FLASH_BANK2_END)
  if (__HAL_FLASH_GET_FLAG(FLASH_FLAG_WRPERR_BANK1) ||
      __HAL_FLASH_GET_FLAG(FLASH_FLAG_PGERR_BANK1) ||
      (__HAL_FLASH_GET_FLAG(FLASH_FLAG_WRPERR_BANK2) ||
       __HAL_FLASH_GET_FLAG(FLASH_FLAG_PGERR_BANK2)))
#else
  ...
  if (pFlash.ProcedureOnGoing == FLASH_PROC_NONE) {
     ...
     */
    __HAL_FLASH_DISABLE_IT(FLASH_IT_EOP_BANK1 | FLASH_IT_ERR_BANK1 |
                           FLASH_IT_EOP_BANK2 | FLASH_IT_ERR_BANK2);
#else
 ...
 *                    (if 0xFFFFFFFF, it means that all the selected pages have
 * been erased)
...
HAL_StatusTypeDef FLASH_WaitForLastOperationBank2(uint32_t Timeout) {
  /* Wait for the FLASH BANK2 operation to complete by polling on BUSY flag to
     be reset. Even if the FLASH BANK2 operation fails, the BUSY flag will be
     reset and an error flag will be set */
  ...
  if (__HAL_FLASH_GET_FLAG(FLASH_FLAG_WRPERR_BANK2) ||
      __HAL_FLASH_GET_FLAG(FLASH_FLAG_PGERR_BANK2)) {
    /*Save the error code*/
    FLASH_SetErrorCode();
    return HAL_ERROR;
