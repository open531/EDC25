...
#define __STM32F1xx_HAL_VERSION                                                \
  ((__STM32F1xx_HAL_VERSION_MAIN << 24) |                                      \
   (__STM32F1xx_HAL_VERSION_SUB1 << 16) |                                      \
   (__STM32F1xx_HAL_VERSION_SUB2 << 8) | (__STM32F1xx_HAL_VERSION_RC))
...
/**
 * @brief  This function is used to initialize the HAL Library; it must be the
 * first instruction to be executed in the main program (before to call any
 * other HAL function), it performs the following: Configure the Flash prefetch.
 *           Configures the SysTick to generate an interrupt each 1 millisecond,
 *           which is clocked by the HSI (at this stage, the clock is not yet
 *           configured and thus the system is running from the internal HSI at
 * 16 MHz). Set NVIC Group Priority to 4. Calls the HAL_MspInit() callback
 * function defined in user file "stm32f1xx_hal_msp.c" to do the global low
...
/**
 * @brief  Returns first word of the unique device identifier (UID based on 96
 * bits)
 * @retval Device identifier
 */
...
/**
 * @brief  Returns second word of the unique device identifier (UID based on 96
 * bits)
 * @retval Device identifier
 */
...
/**
 * @brief  Returns third word of the unique device identifier (UID based on 96
 * bits)
 * @retval Device identifier
 */
