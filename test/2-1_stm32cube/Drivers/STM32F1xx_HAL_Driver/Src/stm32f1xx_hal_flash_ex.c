...
HAL_FLASH_EndOfOperationCallback is called with parameter 0xFFFFFFFF
    ...
    (#) Call the HAL_FLASH_Unlock() function to enable the flash control
register and program memory access.
 ...
 *         contains the configuration information on faulty page in case of
 * error (0xFFFFFFFF means that all the pages have been correctly erased)
 *
  ...
  if (pEraseInit->TypeErase == FLASH_TYPEERASE_MASSERASE) {
    ...
    if (pEraseInit->Banks == FLASH_BANK_BOTH) {
      ...
      if ((FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE) ==
           HAL_OK) &&
          (FLASH_WaitForLastOperationBank2((uint32_t)FLASH_TIMEOUT_VALUE) ==
           HAL_OK)) {
      ...
      /* Wait for last operation to be completed */
      if (FLASH_WaitForLastOperationBank2((uint32_t)FLASH_TIMEOUT_VALUE) ==
          HAL_OK) {
        /*Mass erase to be done*/
      ...
      if (FLASH_WaitForLastOperationBank2((uint32_t)FLASH_TIMEOUT_VALUE) ==
        ...
        *PageError = 0xFFFFFFFFU;
        ...
        for (address = pEraseInit->PageAddress;
             address < (pEraseInit->PageAddress +
          ...
          if (status != HAL_OK) {
            /* In case of error, stop erase procedure and return the faulty
             * address */
            *PageError = address;
            break;
          }
...
#endif /* FLASH_BANK2_END */
    {
      ...
      if (FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE) == HAL_OK) {
        ...
        *PageError = 0xFFFFFFFFU;
        ...
        for (address = pEraseInit->PageAddress;
             address < ((pEraseInit->NbPages * FLASH_PAGE_SIZE) +
...
/** @defgroup FLASHEx_Exported_Functions_Group2 Option Bytes Programming
functions
 ...
 * (RDP). The function @ref HAL_FLASH_Unlock() should be called before to unlock
 * the FLASH interface The function @ref HAL_FLASH_OB_Unlock() should be called
 * before to unlock the options bytes The function @ref HAL_FLASH_OB_Launch()
 * should be called after to force the reload of the options bytes (system reset
 * will occur)
 * @retval HAL status
 */
 ...
 * @note   The function @ref HAL_FLASH_Unlock() should be called before to
 * unlock the FLASH interface The function @ref HAL_FLASH_OB_Unlock() should be
 * called before to unlock the options bytes The function @ref
 * HAL_FLASH_OB_Launch() should be called after to force the reload of the
 * options bytes (system reset will occur)
 *
...
uint32_t HAL_FLASHEx_OBGetUserData(uint32_t DATAAdress) {
  ...
  if (DATAAdress == OB_DATA_ADDRESS_DATA0) {
    ...
    value = READ_BIT(FLASH->OBR, FLASH_OBR_DATA0) >>
            FLASH_POSITION_OB_USERDATA0_BIT;
  } else {
    /* Get value programmed in OB USER Data1 */
    value = READ_BIT(FLASH->OBR, FLASH_OBR_DATA1) >>
 ...
 *         debug features are connected or boot code is executed in RAM, even if
 * nWRPi = 1
 *
...
static HAL_StatusTypeDef FLASH_OB_EnableWRP(uint32_t WriteProtectPage) {
  ...
  if (status == HAL_OK) {
    ...
    /* To be able to write again option byte, need to perform a option byte
     * erase */
 ...
 * @note   Programming of the OB should be performed only after an erase
 * (otherwise PGERR occurs)
 * @param  UserConfig The FLASH User Option Bytes values
 * FLASH_OBR_IWDG_SW(Bit2),
 ...
 * called before to unlock the options bytes The function @ref
 * HAL_FLASH_OB_Launch() should be called after to force the reload of the
 * options bytes (system reset will occur) Programming of the OB should be
 * performed only after an erase (otherwise PGERR occurs)
