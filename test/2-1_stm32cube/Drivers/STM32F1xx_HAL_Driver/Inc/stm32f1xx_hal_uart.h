...
typedef struct {
  ...
  uint32_t
      OverSampling; /*!< Specifies whether the Over sampling 8 is enabled or
                       disabled, to achieve higher speed (up to fPCLK/8). This
                       parameter can be a value of @ref UART_Over_Sampling. This
                       feature is only available on STM32F100xx family, so
                       OverSampling parameter should always be set to 16. */
} UART_InitTypeDef;
...
typedef enum {
  ...
  HAL_UART_STATE_BUSY_TX_RX =
      0x23U, /*!< Data Transmission and Reception process is ongoing
                 ...
                 Value is result of combination (Or) between gState and RxState
                values */
 ...
 *            @arg UART_FLAG_CTS:  CTS Change flag (not available for UART4 and
 * UART5)
 ...
 *          USART_SR register followed by a write operation to USART_DR
 * register.
 ...
 *            @arg UART_IT_ERR:  Error interrupt(Frame error, noise error,
 * overrun error)
 * @retval None
 */
...
/** @brief  Checks whether the specified UART interrupt source is enabled or
 * not.
 * @param  __HANDLE__ specifies the UART Handle.
 *         UART Handle selects the USARTx or UARTy peripheral
 *         (USART,UART availability and x,y values depending on device).
 ...
 *          This parameter can be one of the following values:
 *            @arg UART_IT_CTS: CTS change interrupt (not available for UART4
 * and UART5)
 *            @arg UART_IT_LBD: LIN Break detection interrupt
 *            @arg UART_IT_TXE: Transmit Data Register empty interrupt
 *            @arg UART_IT_TC:  Transmission complete interrupt
...
#define __HAL_UART_GET_IT_SOURCE(__HANDLE__, __IT__)                           \
  (((((__IT__) >> 28U) == UART_CR1_REG_INDEX)                                  \
        ...
        : (((((uint32_t)(__IT__)) >> 28U) == UART_CR2_REG_INDEX)               \
 ...
 * @param  __HANDLE__ specifies the UART Handle.
 *         The Handle Instance can be any USARTx (supporting the HW Flow control
 * feature). It is used to select the USART peripheral (USART availability and x
...
#define IS_UART_PARITY(PARITY)                                                 \
  (((PARITY) == UART_PARITY_NONE) || ((PARITY) == UART_PARITY_EVEN) ||         \
   ((PARITY) == UART_PARITY_ODD))
#define IS_UART_HARDWARE_FLOW_CONTROL(CONTROL)                                 \
  (((CONTROL) == UART_HWCONTROL_NONE) || ((CONTROL) == UART_HWCONTROL_RTS) ||  \
   ((CONTROL) == UART_HWCONTROL_CTS) || ((CONTROL) == UART_HWCONTROL_RTS_CTS))
...
#define IS_UART_OVERSAMPLING(SAMPLING)                                         \
  (((SAMPLING) == UART_OVERSAMPLING_16) || ((SAMPLING) == UART_OVERSAMPLING_8))
#endif /* USART_CR1_OVER8 */
